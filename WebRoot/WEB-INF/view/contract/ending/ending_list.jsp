...
function ending_add() {
	...
	//先打开合同页面，让用户选择
	var win = creatFirstWin('合同信息', 970, 580, 'icon-search', "/ending/endingContract");
	win.window('open');
}
function ending_detail(id) {
		var win = creatWin('合同终止明细', 970, 580, 'icon-search',"/ending/detail/" + id);
		win.window('open');
}
/* function editCF() {
...
function ending_update(id) {
	//var row = $('#ending_dg').datagrid('getSelected');
	var selections = $('#ending_dg').datagrid('getSelections');
	var win = creatWin('合同终止申请', 970, 580, 'icon-search',
			"/ending/edit/" + id);
	win.window('open');
}
