...
<div class="list-div">
	<div style="height: 10px;background-color:#f0f5f7 "></div>
		...
		<ul class="tab-menu">
			<li class="active" onclick="cgReceive(0);">原合同</li>
			<li onclick="cgReceive(1);">変更合同</li>
		</ul>
		<div class="tab-content">
			<div style="height: 440px;">
				<table id="archiving_dg" class="easyui-datagrid"
			data-options="collapsible:true,url:'${base}/Archiving/JsonPaginationsp',
			...
			selectOnCheck: true,checkOnSelect:true,remoteSort:false,nowrap:false,striped: true,fitColumns: true" >
				...
				<tr>
					...
					<th data-options="field:'fReqtIME',align:'center',formatter: ChangeDateFormat,resizable:false,sortable:true" width="12%">申请日期</th>
					<th data-options="field:'fcAmount',align:'center',resizable:false,sortable:true,formatter:function(value,row,index){return fomatMoney(value,2);}" width="14%">合同金额(元)</th>
		...
		<div style="height: 440px;">
			...
			data-options="collapsible:true,url:'${base}/Archiving/htgdsps',
			method:'post',fit:true,pagination:true,singleSelect: true,
			selectOnCheck: true,checkOnSelect:true,remoteSort:false,nowrap:false,striped: true,fitColumns: true" >
				<thead>
					<tr>
						...
						<th data-options="field:'fContName',align:'center',resizable:false,sortable:true" width="15%">変更合同名称</th>
						...
						<th data-options="field:'fAmount',align:'center',resizable:false,sortable:true,formatter:function(value,row,index){return fomatMoney(value,2);}" width="10%">合同金额(元)</th>
						<th data-options="field:'fgdstatus',align:'center',resizable:false,sortable:true,formatter:ContStauts" width="10%">归档状态</th>
						<th data-options="field:'name',align:'center',resizable:false,sortable:true,formatter:CZBG" width="11%">操作</th>
					</tr>
				</thead>
			</table>
		</div>
...
function mouseOut(img) {
	...
	function CZ(val, row) {
		/* if(s==1){
			...
			+ ')" class="easyui-linkbutton"><img onmouseover="arvchivingshowB(this)" onmouseout="arvchivingshowA(this)" src="'+base+'/resource-modality/${themenurl}/list/select1.png">'+ '</a>';
		}else if(s==3||s==9||s==7||s==11||s==0||s==unfiled){
			return 	'<a href="#" onclick="ArchivingHT(' + row.fcId
					+ ')" class="easyui-linkbutton"><img onmouseover="arvchivingshowC(this)" onmouseout="arvchivingshowD(this)" src="'+base+'/resource-modality/${themenurl}/list/archiving1.png">' + '</a>';
		...
		+'<a href="#" onclick="detailInfo(' + row.fcId+ ')" class="easyui-linkbutton"><img onmouseover="listMouseOver(this)" onmouseout="listMouseOut(this)"" src="'+base+'/resource-modality/${themenurl}/list/approval1.png">' + '</a>'
		+'</td></tr></table>';
	}
	    
	function CZBG(val, row) {
		return '<table><tr style="width: 105px;height:20px"><td style="width: 25px">'+
		'<a href="#" onclick="detailInfobg('+row.fId_U+')" class="easyui-linkbutton"><img onmouseover="listMouseOver(this)" onmouseout="listMouseOut(this)" src="'+base+'/resource-modality/${themenurl}/list/select1.png">'+'</a>    '
		+'</td><td style="width: 25px">'
		+'<a href="#" onclick="ArchivingHTbg(' + row.fId_U+ ')" class="easyui-linkbutton"><img onmouseover="listMouseOver(this)" onmouseout="listMouseOut(this)"" src="'+base+'/resource-modality/${themenurl}/list/approval1.png">' + '</a>'
		+'</td></tr></table>';
	}
	...
	function addCF() {
			var win = creatWin('合同归档', 930, 580, 'icon-search',
					"/Archiving/fProCodejsonPagination/");
			win.window('open');
	}
	
	function approval_detail(id) {
		var win = creatWin('合同审批', 1080, 580, 'icon-search',"/Archiving/detailsp/" + id);
		win.window('open');
	}
	
	function detailInfo(id) {
		var win = creatWin('合同归档明细', 1080, 580, 'icon-search',"/Archiving/editsp/" + id);
		win.window('open');
	}
	function detailInfobg(id) {
		var win = creatWin('変更合同归档审批明细', 1080, 580, 'icon-search',"/Archiving/detailbgsp/" + id);
		win.window('open');
	}
	...
	function ArchivingHTbg(id) {
		//var row = $('#archiving_dg').datagrid('getSelected');
		var selections = $('#archiving_dg').datagrid('getSelections');
		var win = creatWin('変更合同归档审批', 1080, 580, 'icon-search',
				"/Archiving/editbgsp/" + id);
		win.window('open');
	}
	function editCF() {
		var row = $('#archiving_dg').datagrid('getSelected');
	...
	function deleteCF() {
		...
		if (row != null && selections.length == 1) {
			if (confirm("确认删除吗？")) {
				$.ajax({
					type : 'POST',
					url : '${base}/Archiving/delete/' + row.fcId,
					dataType : 'json',
