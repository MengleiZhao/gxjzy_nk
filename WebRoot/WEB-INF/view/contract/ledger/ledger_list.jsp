...
<div class="list-div">
	<div style="height: 10px;background-color:#f0f5f7 "></div>
			...
			<table class="top-table" cellpadding="0" cellspacing="0">
				<tr >
					<td class="top-table-search" >合同编号&nbsp;
						...
						&nbsp;&nbsp;合同状态&nbsp;
						<select id="ledger_fContStauts" style="width: 150px;height:25px;" class="easyui-combobox" editable="false">
					...
					<td class="top-table-td2">
						<input id="ledger_fReqtIME" name="" style="width: 150px;height:25px;" class="easyui-datebox"></input>
					</td>
					<td class="top-table-td1" >合同状态</td> 
					<td class="top-table-td2" >
					</td>
		...
		<div class="list-table">
			...
			<c:if test="${!empty fBudgetIndexCode}">data-options="collapsible:true,url:'${base}/Ledger/JsonPagination?fBudgetIndexCode=${fBudgetIndexCode }',</c:if>
			<c:if test="${empty fBudgetIndexCode}">data-options="collapsible:true,url:'${base}/Ledger/JsonPagination',</c:if>
			...
			selectOnCheck: true,checkOnSelect:true,remoteSort:false,nowrap:false,striped: true,fitColumns: true">
				<thead>
					<tr>
						...
						<th data-options="field:'fcCode',align:'center',resizable:false,sortable:true" width="13%">合同编号</th>
						<th data-options="field:'fcTitle',align:'center',resizable:false,sortable:true" width="10%">合同名称</th>
						<th data-options="field:'fSignTime',align:'center',resizable:false,sortable:true,formatter: ChangeDateFormat" width="6%">签订日期</th>
						<th data-options="field:'fcType',align:'center',resizable:false,sortable:true,formatter:returnfcType" width="6%">合同类型</th>
						<th data-options="field:'fContStartTime',align:'center',formatter: ChangeDateFormat,resizable:false,sortable:true" width="6%" >合同开始日期</th>
						<th data-options="field:'fContEndTime',align:'center',formatter: ChangeDateFormat,resizable:false,sortable:true" width="6%" >合同结束日期</th>
						<th data-options="field:'fcAmount',align:'right',resizable:false,sortable:true,formatter:function(value,row,index){return fomatMoney(value,2);}" width="8%">合同金额(元)</th>
						<th data-options="field:'fContractor',align:'center',resizable:false,sortable:true" width="7%" >签约方名称</th>
						<th data-options="field:'fUpdateStatus',align:'center',resizable:false,sortable:true,formatter:HTupdateStatus" width="6%">变更状态</th>
						<th data-options="field:'fDisputeStatus',align:'center',resizable:false,sortable:true,formatter:HTdisputeStatus" width="6%">纠纷状态</th>
						<th data-options="field:'fContStauts1',align:'center',resizable:false,sortable:true,formatter:archiving" width="7%">是否归档</th>
						<th data-options="field:'fDeptName',align:'center',resizable:false,sortable:true" width="7%">承办部门</th>
						<th data-options="field:'fOperator',align:'center',resizable:false,sortable:true" width="5%">申请人</th>
...
function exportContract() {
	...
	function returnfcType(val, row) {
		if (val == 'HTFL-01') {
			return '<span style="color:#666666;">' + " 支出合同" + '</span>';
		}else if (val == 'HTFL-02') {
			return '<span style="color:#666666;">' + " 收入合同" + '</span>';
		}else if (val == 'HTFL-03') {
			return '<span style="color:#666666;">' + " 非经济合同" + '</span>';
		}
	}
	...
	function ledger_detail(id) {
			var win = creatWin('合同台账', 707, 580, 'icon-search',"/Ledger/detail/" + id);
			win.window('open');
	}
	/* function editCF() {
