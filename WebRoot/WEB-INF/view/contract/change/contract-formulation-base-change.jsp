...
<%@ include file="/includes/taglibs.jsp"%>
	<table cellpadding="0" cellspacing="0" border="0">
		...
		<tr class="trbody">
			...
			<td colspan="4">
				<input class="easyui-textbox" type="text"  id="F_fcTitle" name="fcTitle" required="required" data-options="validType:'length[1,50]',onChange:selectmc" style="width: 563px" value="${bean.fcTitle}"/>
			</td>
		</tr>
		<tr class="trbody">
		...
		<tr class="trbody">
			...
			<td class="td2">
				<input id="fperformance" class="easyui-numberbox" required="required" name="fperformance" data-options="icons: [{iconCls:'icon-yuan'}],precision:2,onChange:checkhtjine" style="width: 206px" value="${bean.fperformance}"/>
			</td>
		</tr>
		...
		<tr class="trbody">
			...
			<td colspan="4">
				...
				<input id="f_fRemark" class="easyui-textbox" type="text" required="required" name="fRemark" data-options="multiline:true,onChange:selectsmqk" style="width: 563px;height: 90px" value="${bean.fRemark}"/>
			</td>
		</tr>
		<tr class="trbody">
...
function selectmc(newValue) {
	if(newValue != '${findById.fcTitle}'){
		//$('#F_fcTitle').textbox('textbox').css('background','red');
		$('#F_fcTitle').textbox('textbox').css('color','red');
	}else{
		$('#F_fcTitle').textbox('textbox').css('color','');
	}
}
function checkhtjine(newValue) {
	if(newValue != '${findById.fperformance}'){
		//$('#F_fcTitle').textbox('textbox').css('background','red');
		$('#fperformance').numberbox('textbox').css('color','red');
	}else{
		$('#fperformance').numberbox('textbox').css('color','');
	}
}
...
$("#F_fContStartTime").datebox({
    onSelect : function(beginDate){
    	date=ChangeDateFormatReq(beginDate);
    	date1=ChangeDateFormatReq('${findById.fContStartTime}');
    	 if(date != date1){
    		 $('#F_fContStartTime').datebox('textbox').css('color','red');
    	 }else{
    		 $('#F_fContStartTime').datebox('textbox').css('color','');
    	 }
    },
});
$("#F_fContEndTime").datebox({
    onSelect : function(beginDate){
    	date=ChangeDateFormatReq(beginDate);
    	date1=ChangeDateFormatReq('${findById.fContEndTime}');
    	 if(date != date1){
    		 $('#F_fContEndTime').datebox('textbox').css('color','red');
    	 }else{
    		 $('#F_fContEndTime').datebox('textbox').css('color','');
    	 }
    },
});
function selectsmqk(newValue) {
	if(newValue != '${findById.fRemark}'){
		//$('#F_fcTitle').textbox('textbox').css('background','red');
		$('#f_fRemark').textbox('textbox').css('color','red');
	}else{
		$('#f_fRemark').textbox('textbox').css('color','');
	}
}
</script>