...
<%@ include file="/includes/taglibs.jsp"%>
    <table id="proceeds-edit-plan-dg-change" class="easyui-datagrid" style="width:700px;height:auto;"
			data-options="
				iconCls: 'icon-edit',
				singleSelect: false,
				toolbar: '#tbProceeds',
	...
	/* function isfProceedsProperty(fProceedsProperty){
		var tr = $('#proceeds-edit-plan-dg-change').datagrid('getEditors', editIndexProceeds);
		var text0=tr[0].target.combotree('getText');
		var rows = $('#proceeds-edit-plan-dg-change').datagrid('getRows');
		if(text0 != "阶段款"){
	...
	//完成后计算总额并设置
	var loadplan = 0;
	$('#proceeds-edit-plan-dg-change').datagrid({
		...
		function endEditingPlanProceeds(){
			...
			if ($('#proceeds-edit-plan-dg-change').datagrid('validateRow', editIndexProceeds)){
				...
				$('#proceeds-edit-plan-dg-change').datagrid('endEdit', editIndexProceeds);
		...
		function onClickCellPlan(index, field){
			if (editIndexProceeds != index){
				if (endEditingPlanProceeds()){
					$('#proceeds-edit-plan-dg-change').datagrid('selectRow', index).datagrid('beginEdit', index);
					editIndexProceeds = index;
				} else {
					$('#proceeds-edit-plan-dg-change').datagrid('selectRow', editIndexProceeds);
				}
			}
		}
		function appendPlanProceeds(){
			...
			if (endEditingPlanProceeds()){
				$('#proceeds-edit-plan-dg-change').datagrid('appendRow',{});
				editIndexProceeds = $('#proceeds-edit-plan-dg-change').datagrid('getRows').length-1;
				$('#proceeds-edit-plan-dg-change').datagrid('selectRow', editIndexProceeds)
		...
		function removeitPlanProceeds(){
			...
			for(var i=0;i<obj.length;i++){
			  if(obj[i].checked){
				  $('#proceeds-edit-plan-dg-change').datagrid('cancelEdit', i)
...
function setFsumMoney(newValue,oldValue) {
			...
			var fsumMoney = 0;
			var index=$('#proceeds-edit-plan-dg-change').datagrid('getRowIndex',$('#proceeds-edit-plan-dg-change').datagrid('getSelected'));
			var rows = $('#proceeds-edit-plan-dg-change').datagrid('getRows');
			for(var i=0;i<rows.length;i++){
				if(i == index){
...
function acceptPlanProceeds() {
	if (endEditingPlanProceeds()) {
		$('#proceeds-edit-plan-dg-change').datagrid('acceptChanges');
	}
}
	</script>