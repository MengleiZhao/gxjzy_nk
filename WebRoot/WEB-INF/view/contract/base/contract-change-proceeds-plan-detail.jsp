...
<div class="window-table" style="margin-top: 10px;">
    <table id="proceeds-change-plan-dg" class="easyui-datagrid" style="width:695px;height:auto;"
		...
		<thead>
			<tr>
				<th data-options="field:'fProceedsCondition',editor:{type:'textbox',options:{readonly:true}},validType:'length[1,50]',align:'center'"style="width: 50%">收款条件</th>
				<th data-options="field:'fProceedsTime',align:'center',formatter: ChangeDateFormat,editor:{type:'datebox',options:{readonly:true,precision:1}}"style="width: 25%">预计收款时间</th>
				<th data-options="field:'fProceedsAmount',align:'center',editor:{type:'numberbox',options:{readonly:true,precision:2,onChange:setFsumMoney}}"style="width: 25%">预计收款金额(元)</th>
			</tr>
		</thead>
	</table>
...
function onClickCellPlanProceeds(index, field){
	if (editIndexProceeds != index){
		if (endEditingPlanProceeds()){
			$('#proceeds-change-plan-dg').datagrid('selectRow', index)
...
function removeitPlanProceeds(){
	...
	$('#proceeds-change-plan-dg').datagrid('cancelEdit', editIndexProceeds)
			.datagrid('deleteRow', editIndexProceeds);
	editIndexProceeds = undefined;
	setFsumMoney(0,0);
}
