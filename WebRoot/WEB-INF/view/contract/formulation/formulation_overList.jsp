...
<div class="list-div">
	<div style="height: 10px;background-color:#f0f5f7 "></div>
		...
		<div class="list-table">
			...
			data-options="collapsible:true,url:'${base}/Formulation/OverJsonPagination',
			...
			selectOnCheck: true,checkOnSelect:true,remoteSort:false,nowrap:false,striped: true,fitColumns: true">
				<thead>
					<tr>
						...
						<th data-options="field:'fSignTime',align:'center',resizable:false,sortable:true,formatter: ChangeDateFormat" width="7%">签订日期</th>
						...
						<th data-options="field:'fcAmount',align:'right',resizable:false,sortable:true,formatter:function(value,row,index){return fomatMoney(value,2);}" width="9%">合同金额(元)</th>
						...
						<th data-options="field:'fOperator',align:'center',resizable:false,sortable:true" width="7%">申请人</th>
...
function ledger_clearTable() {
	...
	$('#ledger_fContStauts').combobox('setValue',null);
	queryledger();
}
//鼠标移入图片替换
...
function exportContract() {
	...
	function ContStauts(val, row) {
		if (val == 1) {
			...
			return '<span style="color:#666666;"><img src="'+base+'/resource-modality/${themenurl}/ytg.png">' + " 已完结" + '</span>';
		}else if (val == 5) {
	...
	function CZ(val, row) {
		if(row.fContStauts==3){
			...
			return 	'<a href="#" onclick="ledger_detail(' + row.fcId+ ')" class="easyui-linkbutton"><img onmouseover="listMouseOver(this)" onmouseout="listMouseOut(this)" src="'+base+'/resource-modality/${themenurl}/list/select1.png">' + '</a>' 
			+'&nbsp;&nbsp;&nbsp;&nbsp;'
			+'<a href="#" onclick="over(' + row.fcId
			+ ')" class="easyui-linkbutton"><img  onmouseover="listMouseOver(this)" onmouseout="listMouseOut(this)" src="'+base+'/resource-modality/${themenurl}/list/wanjie1.png">'
		}
	}
	...
	function over(id){
		if (confirm("确认是否完结？")) {
			$.ajax({
				type : 'POST',
				url : '${base}/Formulation/over?id='+id,
				dataType : 'json',
				success : function(data) {
					if (data.success) {
						$.messager.alert('系统提示', data.info, 'info');
