...
<div class="list-div">
	<div style="height: 10px;background-color:#f0f5f7 "></div>
		<div class="list-top">
			...
			selectOnCheck: true,checkOnSelect:true,remoteSort:true,nowrap:false,striped: true,fitColumns: true,onLoadSuccess:onLoadSuccessUserCheckReim" >
				<thead>
					<tr>
...
function onLoadSuccessUserCheckReim(){
	var peopId = '${peopId}';
	if(peopId==''){
		return false;
	}
	var peopIds = peopId.split(",");
	var rows = $('#choose_user_rdg').datagrid('getRows');
	for (var i = 0; i < peopIds.length; i++) {
		for (var j = 0; j < rows.length; j++) {
			if(peopIds[i]==rows[j].id){
				$('#choose_user_rdg').datagrid('selectRow',j);
			}
		}
	}
}
