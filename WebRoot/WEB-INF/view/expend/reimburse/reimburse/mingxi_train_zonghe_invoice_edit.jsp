...
<form id="form1">
	<table id="zonghefapiao" class="window-table-readonly-zc" cellspacing="0" cellpadding="0" style=" height: 100px;width:695px;">
		<c:forEach items="${Invoicelist1 }" varStatus="i" var="invoice">
			<tbody id="zonghe_${i.index}" >
				<tr>
					<td class="td1" style="width: 100px;">
						&nbsp;&nbsp;<span id="zongheFp_${i.index}">发票${i.index+1}</span>
						<input type="text" id="zonghefp_${i.index}" value="${invoice.fileId}" name="fileId" hidden="hidden">
						<input type="file" id="zongheimage_${i.index}" accept="image/jpeg,image/jpg,image/png" onchange="upImageFilezonghe(this)" hidden="hidden">
					</td>
					<td class="td2" colspan="3" id="zongheimagetd_${i.index}" style="width: 150px;">
						<c:if test="${invoice.fileId!=null&&!empty invoice.fileId}">
							...
							<a style="color:red" href="#" onclick="deleteImagezonghe(this)" id="zonghesc_${i.index}">    删除</a>
						</c:if>
						<c:if test="${invoice.fileId==null||empty invoice.fileId}">
						<a onclick="clickUpFilezonghe(this)" id="zongheclick_${i.index}" style="font-weight: bold;" href="#">
					...
					<td class="td2" colspan="3" id="">
						<div style="width: auto; height:auto; height:auto;margin-top:5px;margin-left:49px;width: 300px;">
							<p style="margin-bottom:10px;">
								...
								<input class="input_amonut1 a easyui-numberbox" name="amount" id="zongheamount_${i.index}" value="${invoice.amount}" onchange="addAmonut2(this)"data-options="precision:2"/>
								<!-- oninput = "value=value.replace(/^\D*(\d*(?:\.\d{0,2})?).*$/g, '$1')" value=""  -->
							</p>
							<p style="margin-bottom:5px;" >
								<span class="zonghefp_span">备注：</span>
								<input  class="input_amonut1 easyui-textbox" name="remark" id="zongheremark_${i.index}"value="${invoice.remark}" />
							</p>
						</div>
					</td>
...
function countDays2(obj){
	...
	if (d > 0) {
		var amount =parseFloat($('#zongheamount_' + i).val());
		if(!isNaN(amount)){
...
function appendzonghe(obj) {
	...
	'<td class="td2" colspan="3" id="zongheimagetd_'+a+'" style="width: 150px;">'+
	...
	'<p style="margin-bottom:10px;">'+
	'<span class="zonghefp_span" >金额(元)：</span>'+
	'<input class="input_amonut1 a easyui-numberbox" name="amount" id="zongheamount_'+a+'" onchange="addAmonut2(this)"data-options="precision:2"/>'+
	'</p>'+
	'<p style="margin-bottom:5px;" >'+
	'<span class="zonghefp_span">备注：</span>'+
		...
		$.ajax({
					...
					success : function(data) {
			        	...
			        	if(y!=0){
			        		$('#zongheimagetd_' + j)
							.append(
									'<img style="vertical-align:bottom;margin-left:0px; width: 100px;height: 73px" src="'+data+'" onclick="yl(this.src)"/><a style="color:red" href="#" id="zonghesc_'
											+ j
											+ '" onclick="deleteImagezonghe(this)">    删除</a>');
				        	}else{
				        		$('#zongheimagetd_'+j).append('<img style="vertical-align:bottom;margin-left:0px; width: 100px;height: 73px" src="'+data+'" onclick="yl(this.src)"/><a style="color:red" href="#" id="zonghesc_'+j+'" onclick="deleteImagezonghe(this)">    删除</a>');
				        	}
	...
	var id =obj.id;
     ...
     var y=num.substring(x+1,num.length);
		...
		.append(
     		...
     		$('#zongheimagetd_'+j).append('<a onclick="clickUpFilezonghe(this)" id="zongheclick_'+j+'" style="font-weight: bold;" href="#">'+
