...
<form id="reimburse_save_form" method="post" data-options="novalidate:true" class="easyui-form" enctype="multipart/form-data">
				...
				<div id="spbxjbxx" style="overflow:auto; margin-top: -15px;">
					...
					<table class="window-table" style="margin-top: 10px" cellspacing="0" cellpadding="0">
						...
						<%-- <tr class="trbody">
							...
							<td class="td2">
							<select id="reimburseType" class="easyui-combobox" name="type" style="width: 200px;height: 30px;">
							...
							<td class="td2" style="text-align: right;">
								<input class="easyui-numberbox" id="reimburseAmount" name="amount" style="width: 200px;height: 30px;text-align: right" data-options="precision:2,icons: [{iconCls:'icon-yuan'}],required:true"
						...
						<tr class="trbody">
							...
							<td colspan="4">
								<a href="#" onclick="openApply(${bean.type})">
					...
					<table class="window-table" cellspacing="0" cellpadding="0">
						...
						<tr class="trbody" style="margin-top: 10px;">
							...
							<td colspan="4">
								<%-- <input class="easyui-textbox" data-options="multiline:true,required:true,validType:'length[0,250]'" name="reimburseReason" style="width:590px;height:70px;" 
	...
	<input type="hidden" name=name value="2" id="input_deptName"/>
	<input type="hidden" name=name value="4" id="input_deptName"/>
	<input type="hidden" name=name value="6" id="input_deptName"/>
	<input type="hidden" name=name value="7" id="input_deptName"/>
	<input type="hidden" name=name value="9" id="input_deptName"/>
	<input type="hidden" name=name value="5" id="input_deptName"/>
	</form>
...
$(document).ready(function() {
	...
	var formData = $('form[id="test"]').serialize();
	var formDataArr = $('form[id="test"]').serializeArray();
	...
	$.ajax({
	      ...
	      success:function(data){
	    		...
	    		$('#reimburse_save_form').form('load', data);
	    	  	...
	    	  	//预算批复金额
	    	  	$('#p_pfAmount').html(data.pfAmount+"元");
	    	  	//预算批复时间
	    	  	$('#p_pfDate').html(data.pfDate);
	    	  	//使用部门
	    	  	$('#p_pfDepartName').html(data.pfDepartName);
	    	  	//可用金额
	    	  	$('#p_syAmount').html(data.syAmount+"元");
