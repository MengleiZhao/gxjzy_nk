...
<form id="form2">
	<table id="hotelAttendfapiao" class="window-table-readonly-zc" cellspacing="0" cellpadding="0" style=" height: 100px;width:695px;">
		<c:forEach items="${Invoicelist2 }" varStatus="i" var="invoice">
			<tbody id="hotelAttend_${i.index}" >
				<tr>
					<td class="td1" style="width: 100px;">
						&nbsp;&nbsp;<span id="hotelAttendFp_${i.index}">发票${i.index+1}</span>
						<input type="text" id="hotelAttendfp_${i.index}" value="${invoice.fileId}" name="fileId" hidden="hidden">
						<input type="file" id="hotelAttendimage_${i.index}" accept="image/jpeg,image/jpg,image/png" onchange="upImageFilehotelAttend(this)" hidden="hidden">
					</td>
					<td class="td2" colspan="3" id="hotelAttendimagetd_${i.index}" style="width: 150px;">
						<c:if test="${invoice.fileId!=null&&!empty invoice.fileId}">
							...
							<a style="color:red" href="#" onclick="deleteImagehotelAttend(this)" id="hotelAttendsc_${i.index}">    删除</a>
						</c:if>
						<c:if test="${invoice.fileId==null||empty invoice.fileId}">
						<a onclick="clickUpFile2(this)" id="hotelAttendclick_${i.index}" style="font-weight: bold;" href="#">
					...
					<td class="td2" colspan="3" id="">
						<div style="width: auto; height:auto; height:auto;margin-top:5px;margin-left:49px;width: 300px;">
							<p style="margin-bottom:10px;">
								<span class="hotelAttendfp_span" >金额(元)：</span>
								<input class="input_amonut1 a easyui-numberbox" name="amount" id="hotelAttendamount_${i.index}" value="${invoice.amount}" onchange="addAmonut2(this)"data-options="precision:2"/>
								<!-- oninput = "value=value.replace(/^\D*(\d*(?:\.\d{0,2})?).*$/g, '$1')" value=""  -->
							</p>
							<p style="margin-bottom:5px;" >
								<span class="hotelAttendfp_span">备注：</span>
								<input  class="input_amonut1 easyui-textbox" name="remark" id="hotelAttendremark_${i.index}"value="${invoice.remark}" />
							</p>
						</div>
					</td>
					<td style="vertical-align:top">
						<div>
							<c:if test="${i.index!=0 }">
								<a onclick="deletehotelAttend(this)" style="float: right;" href="#" > 
	...
	<a onclick="appendhotelAttendEditTravel(this)" style="font-weight: bold;vertical-align: middle;margin-left: 308px;" href="#">
...
function countDays2(obj){
	...
	if (d > 0) {
		var amount =parseFloat($('#hotelAttendamount_' + i).val());
		if(!isNaN(amount)){
...
function countTravel2(obj){
	...
	var i=id.substring(j+1,id.length);
	var num =0;
	var num1 =parseFloat($('#hotelAttendamount_' + i).val());
...
function appendhotelAttendEditTravel(obj) {
	index = index + 1;
	var fromNum = parseInt(obj.parentNode.parentNode.children[5].children[0].children.length);
	$('#hotelAttendfapiao').append('<tbody id="hotelAttend_'+a+'" >'+
	'<tr><td class="td1" style="width: 100px;">&nbsp;&nbsp;<span id="hotelAttendFp_'+a+'">发票'+(fromNum+1)+'</span>'+
	'<input type="text" id="hotelAttendfp_'+a+'" name="fileId" hidden="hidden">'+
	'<input type="file" id="hotelAttendimage_'+a+'" accept="image/jpeg,image/jpg,image/png" onchange="upImageFilehotelAttend(this)" hidden="hidden"></td>'+
	'<td class="td2" colspan="3" id="hotelAttendimagetd_'+a+'" style="width: 150px;">'+
	'<a onclick="clickUpFile2(this)" id="hotelAttendclick_'+a+'" style="font-weight: bold;" href="#">'+
	'<img style="vertical-align:bottom;margin-left:30px" src="${base}/resource-modality/${themenurl}/button/shangchuan1.png" onMouseOver="mouseOver(this)" onMouseOut="mouseOut(this)">'+
	'</a></td>'+
	'<td class="td2" colspan="3" id="">'+
	'<div style="width: auto; height:auto; height:auto;margin-top:5px;margin-left:49px;width: 300px;">'+
	'<p style="margin-bottom:10px;">'+
	'<span class="hotelAttendfp_span" >金额(元)：</span>'+
	'<input class="input_amonut1 a easyui-numberbox" name="amount" id="hotelAttendamount_'+a+'" onchange="addAmonut2(this)"data-options="precision:2"/>'+
	'</p>'+
	'<p style="margin-bottom:5px;" >'+
	'<span class="hotelAttendfp_span">备注：</span>'+
	'<input  class="input_amonut1 easyui-textbox" name="remark" id="hotelAttendremark_'+a+'" value="" />'+
	'</p></div>'+
	'</td>'+
	'<td style="vertical-align:top">'+
	'<div>'+
	...
	'</td>'+
	'</tr></tbody>');
	$.parser.parse($('#hotelAttend_'+a).parent());
...
function clickUpFile2(obj) {
	var id = obj.id;
	var i = id.lastIndexOf("\_");
	var j = id.substring(i + 1, id.length);
	$('#hotelAttendimage_' + j).click();
}

//上传图片文件转化为base64在页面显示
function upImageFilehotelAttend(obj) {
	var id = obj.id;
	var i = id.lastIndexOf("\_");
	var j = id.substring(i + 1, id.length);
	if (obj == null || obj == "") {
	...
	for (var i = 0; i < files.length; i++) {
		...
		$.ajax({
					...
					success : function(data) {
						upladFp(obj, "hotelAttend", "fp04");
			        	var num = obj.parentNode.parentNode.parentNode.id;
			        	$('#hotelAttendimagetd_'+j).empty();
			        	$('#hotelAttendimage_'+j).val("");
			        	var x=num.lastIndexOf("\_");
			   	     	var y=num.substring(x+1,num.length);
			        	if(y!=0){
			        		$('#hotelAttendimagetd_' + j)
							.append(
									'<img style="vertical-align:bottom;margin-left:0px; width: 100px;height: 73px" src="'+data+'" onclick="yl(this.src)"/><a style="color:red" href="#" id="hotelAttendsc_'
											+ j
											+ '" onclick="deleteImagehotelAttend(this)">    删除</a>');
				        	}else{
				        		$('#hotelAttendimagetd_'+j).append('<img style="vertical-align:bottom;margin-left:0px; width: 100px;height: 73px" src="'+data+'" onclick="yl(this.src)"/><a style="color:red" href="#" id="hotelAttendsc_'+j+'" onclick="deleteImagehotelAttend(this)">    删除</a>');
				        	}
...
//删除上传的图片
function deleteImagehotelAttend(obj) {
	...
	var id =obj.id;
     ...
     var y=num.substring(x+1,num.length);
   	  if(y!=0){
   		$('#hotelAttendimagetd_' + j)
		.append(
				'<a onclick="clickUpFile2(this)" id="hotelAttendclick_'
						+ j
     		...
     		$('#hotelAttendimagetd_'+j).append('<a onclick="clickUpFile2(this)" id="hotelAttendclick_'+j+'" style="font-weight: bold;" href="#">'+
...
function deletehotelAttend(item){
	...
	for(var i=1;i<fromNum;i++){
		if('hotelAttend'==arr[0]){
			var tableId = $('#hotelAttendfapiao').find('tbody');
