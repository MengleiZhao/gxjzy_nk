...
<form id="form4">
			...
			<tbody id="fete_${i.index}" >
				<tr>
					<td class="td1" style="width: 100px;">
						...
						<input type="file" id="image_${i.index}" accept=",image/jpeg,image/jpg,image/png" onchange="upImageFilefete(this)" hidden="hidden">
					</td>
					<td class="td2" colspan="3" id="zfbimagetd_${i.index}" style="width: 150px;">
						<c:if test="${invoice.fileId!=null&&!empty invoice.fileId}">
							<img style="vertical-align:bottom;width: 100px; height: 73px;margin-left:0px" onclick="yl(this.src)" src="${base}/attachment/download/${invoice.fileId}"/><a style="color:red" href="#" onclick="deleteImagefete(this)" id="fetesc_${i.index}">    删除</a>
						</c:if>
						<c:if test="${invoice.fileId==null||empty invoice.fileId}">
							<a onclick="clickUpFilefete(this)" id="clickfete_${i.index}" style="font-weight: bold;" href="#">
	...
	<a onclick="appendfete(this)" style="font-weight: bold;vertical-align: middle;margin-left: 308px;" href="#">
...
function addAmonut(obj){
    ...
    $('#reimburseAmount').val(totalAmount);
      countDays(obj);
      cx();
}
...
function countTravel(obj){
	...
	function appendfete(obj) {
		...
		'<input type="file" id="image_'+a+'" accept="image/jpeg,image/jpg,image/png" onchange="upImageFilefete(this)" hidden="hidden"></td>'+
