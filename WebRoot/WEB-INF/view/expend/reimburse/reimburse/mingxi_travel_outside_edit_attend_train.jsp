...
<form id="form1">
			...
			<tbody id="outsideAttend_${i.index}" >
				<tr>
					<td class="td1" style="width: 100px;">
						...
						<input type="text" id="outsideAttendfp_${i.index}" value="${invoice.fileId}" name="fileId" hidden="hidden">
						<input type="file" id="outsideAttendImage_${i.index}" accept="image/jpeg,image/jpg,image/png" onchange="upImageFileoutsideAttend(this)" hidden="hidden">
					</td>
					<td class="td2" colspan="3" id="outsideAttendZfbimagetd_${i.index}" style="width: 150px;">
						<c:if test="${invoice.fileId!=null&&!empty invoice.fileId}">
							<img style="vertical-align:bottom;width: 100px; height: 73px;margin-left:0px" onclick="yl(this.src)" src="${base}/attachment/download/${invoice.fileId}"/><a style="color:red" href="#" onclick="deleteImageoutsideAttend(this)" id="outsideAttendsc_${i.index}">    删除</a>
						</c:if>
						<c:if test="${invoice.fileId==null||empty invoice.fileId}">
							<a onclick="clickUpFileoutsideAttend(this)" id="clickoutsideAttend_${i.index}" style="font-weight: bold;" href="#">
	...
	<a onclick="appendoutsideAttendEdit(this)" style="font-weight: bold;vertical-align: middle;margin-left: 308px;" href="#">
...
function countTravel(obj){
	...
	function appendoutsideAttendEdit(obj) {
		index = index + 1;
		...
		'<a onclick="clickUpFileoutsideAttend(this)" id="clickoutsideAttend_'+a+'" style="font-weight: bold;" href="#">'+
		'<img style="vertical-align:bottom;margin-left:30px" src="${base}/resource-modality/${themenurl}/button/shangchuan1.png" onMouseOver="mouseOver(this)" onMouseOut="mouseOut(this)">'+
		'</a></td>'+
		'<td class="td2" colspan="3" id="">'+
		'<div style="width: auto; height:auto; height:auto;margin-top:5px;margin-left:49px;width: 300px;">'+
			...
			$.ajax({
						...
						success : function(data) {
				        	...
				        	if(y!=0){
								...
								.append(
										'<img style="vertical-align:bottom;margin-left:0px; width: 100px;height: 73px" src="'+data+'" onclick="yl(this.src)"/><a style="color:red" href="#" id="outsideAttendsc_'
												+ j
												+ '" onclick="deleteImageoutsideAttend(this)">    删除</a>');
					        	}else{
					        		$('#outsideAttendZfbimagetd_'+j).append('<img style="vertical-align:bottom;margin-left:0px; width: 100px;height: 73px" src="'+data+'" onclick="yl(this.src)"/><a style="color:red" href="#" id="outsideAttendsc_'+j+'" onclick="deleteImageoutsideAttend(this)">    删除</a>');
					        	}
		...
		var id =obj.id;
	     ...
	     var y=num.substring(x+1,num.length);
	   	  if(y!=0){
	   		$('#outsideAttendZfbimagetd_' + j)
			.append(
					'<a onclick="clickUpFileoutsideAttend(this)" id="clickoutsideAttend_'
							+ j
      		...
      		$('#outsideAttendZfbimagetd_'+j).append('<a onclick="clickUpFileoutsideAttend(this)" id="clickoutsideAttend_'+j+'" style="font-weight: bold;" href="#">'+
