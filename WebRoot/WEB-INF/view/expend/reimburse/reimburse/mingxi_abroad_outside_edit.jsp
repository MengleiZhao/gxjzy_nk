...
<form id="form2">
	<table id="outsidefapiao" class="window-table-readonly-zc" cellspacing="0" cellpadding="0" style=" height: 100px;width:695px;">
		<c:forEach items="${Invoicelist2 }" varStatus="i" var="invoice">
			<tbody id="outside_${i.index}" >
				<tr>
					<td class="td1" style="width: 100px;">
						&nbsp;&nbsp;<span id="outsideFp_${i.index}">发票${i.index+1}</span>
						<input type="text" id="outsidefp_${i.index}" value="${invoice.fileId}" name="fileId" hidden="hidden">
						<input type="file" id="image_${i.index}" accept="image/jpeg,image/jpg,image/png" onchange="upImageFileOutside(this)" hidden="hidden">
					</td>
					<td class="td2" colspan="3" id="zfbimagetd_${i.index}" style="width: 150px;">
						<c:if test="${invoice.fileId!=null&&!empty invoice.fileId}">
							<img style="vertical-align:bottom;width: 100px; height: 73px;margin-left:0px" onclick="yl(this.src)" src="${base}/attachment/download/${invoice.fileId}"/><a style="color:red" href="#" onclick="deleteImageOutside(this)" id="outsidesc_${i.index}">    删除</a>
						</c:if>
						<c:if test="${invoice.fileId==null||empty invoice.fileId}">
							<a onclick="clickUpFileOutside(this)" id="clickOutside_${i.index}" style="font-weight: bold;" href="#">
	...
	<a onclick="appendOutsideAbrEdit(this)" style="font-weight: bold;vertical-align: middle;margin-left: 308px;" href="#">
...
function countTravel(obj){
	...
	function appendOutsideAbrEdit(obj) {
		index = index + 1;
		...
		'<input type="file" id="image_'+a+'" accept="image/jpeg,image/jpg,image/png" onchange="upImageFileOutside(this)" hidden="hidden"></td>'+
		'<td class="td2" colspan="3" id="zfbimagetd_'+a+'" style="width: 150px;">'+
		'<a onclick="clickUpFileOutside(this)" id="clickOutside_'+a+'" style="font-weight: bold;" href="#">'+
		'<img style="vertical-align:bottom;margin-left:30px" src="${base}/resource-modality/${themenurl}/button/shangchuan1.png" onMouseOver="mouseOver(this)" onMouseOut="mouseOut(this)">'+
		'</a></td>'+
		'<td class="td2" colspan="3" id="">'+
		'<div style="width: auto; height:auto; height:auto;margin-top:5px;margin-left:49px;width: 300px;">'+
			...
			$.ajax({
						...
						success : function(data) {
				        	...
				        	if(y!=0){
				        		$('#zfbimagetd_' + j)
								.append(
										'<img style="vertical-align:bottom;margin-left:0px; width: 100px;height: 73px" src="'+data+'" onclick="yl(this.src)"/><a style="color:red" href="#" id="outsidesc_'
												+ j
												+ '" onclick="deleteImageOutside(this)">    删除</a>');
					        	}else{
					        		$('#zfbimagetd_'+j).append('<img style="vertical-align:bottom;margin-left:0px; width: 100px;height: 73px" src="'+data+'" onclick="yl(this.src)"/><a style="color:red" href="#" id="outsidesc_'+j+'" onclick="deleteImageOutside(this)">    删除</a>');
					        	}
		...
		var id =obj.id;
	     ...
	     var y=num.substring(x+1,num.length);
	   	  if(y!=0){
	   		$('#zfbimagetd_' + j)
			.append(
					'<a onclick="clickUpFileOutside(this)" id="clickOutside_'
							+ j
      		...
      		$('#zfbimagetd_'+j).append('<a onclick="clickUpFileOutside(this)" id="clickOutside_'+j+'" style="font-weight: bold;" href="#">'+
