...
<form id="form1">
	<table id="mixfapiao" class="window-table-readonly-zc" cellspacing="0" cellpadding="0" style=" height: 100px;width:695px;">
		<c:forEach items="${Invoicelist1 }" varStatus="i" var="invoice">
			<tbody id="mix_${i.index}" >
				<tr>
					<td class="td1" style="width: 100px;">
						&nbsp;&nbsp;<span id="mixFp_${i.index}">发票${i.index+1}</span>
						<input type="text" id="mixfp_${i.index}" value="${invoice.fileId}" name="fileId" hidden="hidden">
						<input type="file" id="mixImage_${i.index}" accept="image/jpeg,image/jpg,image/png" onchange="upImageFilemix(this)" hidden="hidden">
					</td>
					<td class="td2" colspan="3" id="mixZfbimagetd_${i.index}" style="width: 150px;">
						<c:if test="${invoice.fileId!=null&&!empty invoice.fileId}">
							<img style="vertical-align:bottom;width: 100px; height: 73px;margin-left:0px" onclick="yl(this.src)" src="${base}/attachment/download/${invoice.fileId}"/><a style="color:red" href="#" onclick="deleteImagemix(this)" id="mixsc_${i.index}">    删除</a>
						</c:if>
						<c:if test="${invoice.fileId==null||empty invoice.fileId}">
							<a onclick="clickUpFilemix(this)" id="clickmix_${i.index}" style="font-weight: bold;" href="#">
					...
					<td style="vertical-align:top">
						<div>
							<c:if test="${i.index!=0}">
								<a onclick="deletemix(this)" style="float: right;" href="#" > 
	...
	<a onclick="appendmix(this)" style="font-weight: bold;vertical-align: middle;margin-left: 308px;" href="#">
...
function countTravel(obj){
	...
	function appendmix(obj) {
		index = index + 1;
		var fromNum = parseInt(obj.parentNode.parentNode.children[5].children[0].children.length);
		$('#mixfapiao').append('<tbody id="mix_'+a+'" >'+
		'<tr><td class="td1" style="width: 100px;">&nbsp;&nbsp;<span id="mixFp_'+a+'">发票'+(fromNum+1)+'</span>'+
		'<input type="text" id="mixfp_'+a+'" name="fileId" hidden="hidden">'+
		'<input type="file" id="mixImage_'+a+'" accept="image/jpeg,image/jpg,image/png" onchange="upImageFilemix(this)" hidden="hidden"></td>'+
		'<td class="td2" colspan="3" id="mixZfbimagetd_'+a+'" style="width: 150px;">'+
		'<a onclick="clickUpFilemix(this)" id="clickmix_'+a+'" style="font-weight: bold;" href="#">'+
		'<img style="vertical-align:bottom;margin-left:30px" src="${base}/resource-modality/${themenurl}/button/shangchuan1.png" onMouseOver="mouseOver(this)" onMouseOut="mouseOut(this)">'+
		'</a></td>'+
		'<td class="td2" colspan="3" id="">'+
		'<div style="width: auto; height:auto; height:auto;margin-top:5px;margin-left:49px;width: 300px;">'+
			...
			$.ajax({
						...
						success : function(data) {
				        	...
				        	if(y!=0){
				        		$('#mixZfbimagetd_' + j)
								.append(
										'<img style="vertical-align:bottom;margin-left:0px; width: 100px;height: 73px" src="'+data+'" onclick="yl(this.src)"/><a style="color:red" href="#" id="mixsc_'
												+ j
												+ '" onclick="deleteImagemix(this)">    删除</a>');
					        	}else{
					        		$('#mixZfbimagetd_'+j).append('<img style="vertical-align:bottom;margin-left:0px; width: 100px;height: 73px" src="'+data+'" onclick="yl(this.src)"/><a style="color:red" href="#" id="mixsc_'+j+'" onclick="deleteImagemix(this)">    删除</a>');
					        	}
		...
		var id =obj.id;
	     ...
	     var y=num.substring(x+1,num.length);
	   	  if(y!=0){
	   		$('#mixZfbimagetd_' + j)
			.append(
					'<a onclick="clickUpFilemix(this)" id="clickmix_'
							+ j
      		...
      		$('#mixZfbimagetd_'+j).append('<a onclick="clickUpFilemix(this)" id="clickmix_'+j+'" style="font-weight: bold;" href="#">'+
	...
	function  deletemix(item){
		...
		for(var i=1;i<fromNum;i++){
			if('mix'==arr[0]){
				var tableId = $('#mixfapiao').find('tbody');
