...
<%@ include file="/includes/taglibs.jsp"%>
			<form id="reimburse_save_form" method="post"  enctype="multipart/form-data">
				...
				<!-- 收款人信息 -->
				<input hidden="hidden" id="payeeAmount" name="payeeAmount" value="${payee.payeeAmount}"/>
				...
				<!-- 国际旅费  --><input type="hidden" id="travelMoney" name="travelMoney" value="${abroadEdit.travelMoney}" />
				<!-- 住宿费  --><input type="hidden" id="hotelMoney" name="hotelMoney" value="${abroadEdit.hotelMoney}"/>
				<!-- 伙食费  --><input type="hidden" id="foodMoney" name="foodMoney"  value="${abroadEdit.foodMoney}"/>
				<!-- 公杂费  --><input type="hidden" id="mixMoney" name="mixMoney" value="${abroadEdit.mixMoney}" />
				<!-- 宴请费  --><input type="hidden" id="feteMoney" name="feteMoney"  value="${abroadEdit.feteMoney}"/>
				<!-- 国外交通费  --><input type="hidden" id="trafficMoney" name="trafficMoney"  value="${abroadEdit.trafficMoney}"/>
				<!-- 其他费用  --><input type="hidden" id="totalOtherMoney" name="totalOtherMoney"  value="${abroadEdit.totalOtherMoney}"/>
				...
				<!-- 出国人员信息  --><input type="hidden" id="abroadPeople" name="abroadPeopleJson" />
				</div>
				<!-- 基本信息 -->
				...
				<div title="基本信息" data-options=" collapsible:false" style="overflow:auto;width: 717px">
					<table class="window-table" style="margin-top: 10px;width: 695px" cellspacing="0" cellpadding="0">
								<tr class="trbody">
									...
									<td colspan="4"  >
										<input style="width: 595px;height: 30px;" name="gCode" class="easyui-textbox"
								...
								<tr class="trbody">
									...
									<td colspan="4"  >
										<input class="easyui-textbox" style="width: 595px;height: 30px; " value="${bean.gName}" name="gName" data-options="validType:'length[1,50]'"/> 
									</td>
								</tr>
								<tr class="trbody">
									<td class="td1" style="width:70px;"><span class="style1">*</span>团组名称</td>
									<td class="td2" >
										<input style="width: 230px; height: 30px;" id="fTeamName" name="fTeamName" class="easyui-textbox" <c:if test="${bean.fupdateStatus=='0'}"> readonly="readonly" </c:if>
									...
									<td class="td2">
										<input style="width: 236px; height: 30px;" id="fAbroadPlace" name="fAbroadPlace" class="easyui-textbox" <c:if test="${bean.fupdateStatus=='0'}"> readonly="readonly" </c:if>
								...
								<tr class="trbody">
									...
									<td class="td2">
										<input style="width: 230px; height: 30px;" id="fTeamLeader" name="fTeamLeader" class="easyui-textbox" <c:if test="${bean.fupdateStatus=='0'}"> readonly="readonly" </c:if> 
										value="${abroadEdit.fTeamLeader}"  data-options="required:true,validType:'length[1,100]'"/> 
									</td>
									<td  style="width:70px;float: right;"></td>
									<td class="td2">
									</td>
								</tr>
								<tr class="trbody">
									...
									<td class="td2">
										<input style="width: 230px; height: 30px;" id="fTeamPersonNum"  name="fTeamPersonNum" class="easyui-numberbox" <c:if test="${bean.fupdateStatus=='0'}"> readonly="readonly" </c:if>
										value="${abroadEdit.fTeamPersonNum}" data-options="required:true,validType:'length[1,100]'"/> 
									</td>
									<td class="td1" style="width:100px;"><span class="style1">*</span>本单位参团人数</td>
									<td class="td2">
										<input style="width: 236px; height: 30px;" id="fThisUnitTeamPersonNum"  name="fThisUnitTeamPersonNum" class="easyui-numberbox" <c:if test="${bean.fupdateStatus=='0'}"> readonly="readonly" </c:if> 
										value="${abroadEdit.fThisUnitTeamPersonNum}" data-options="required:true,validType:'length[1,100]'"/> 
									</td>
								</tr>
								<tr class="trbody">
									<td class="td1" style="width:70px;"><span class="style1">*</span>开始时间</td>
									<td class="td2">
										<input style="width: 230px; height: 30px;" id="abroadDateStart" name="fAbroadDateStart" class="easyui-datebox" <c:if test="${bean.fupdateStatus=='0'}"> readonly="readonly" </c:if> 
									...
									<td class="td2">
										<input style="width: 236px; height: 30px;" id="abroadDateEnd" name="fAbroadDateEnd" class="easyui-datebox"   <c:if test="${bean.fupdateStatus=='0'}"> readonly="readonly" </c:if> 
										value="${abroadEdit.fAbroadDateEnd}" data-options="onSelect:onSelectAbroad2,required:true" editable="false"/>
									</td>
								</tr>
								<tr class="trbody">
									<td class="td1" style="width:70px;"><span class="style1">*</span>出国天数</td>
									<td class="td2" colspan="3">
										<input style="width: 230px; height: 30px;" id="abroadDay" name="fAbroadDayNum" class="easyui-textbox"  <c:if test="${bean.fupdateStatus=='0'}"> readonly="readonly" </c:if> 
								...
								<tr class="trbody">
									...
									<td class="td2" colspan="3">
			                       		<input type="radio" name="fDiningPlaceChecked" id="fDiningPlaceId1"  <c:if test="${bean.fupdateStatus=='0'}"> disabled="disabled" </c:if> onclick="checkedFeteA()" value="1" <c:if test="${abroadEdit.fDiningPlace==1} "> checked </c:if>>是
			                        	<input type="radio" name="fDiningPlaceChecked" id="fDiningPlaceId2"  <c:if test="${bean.fupdateStatus=='0'}"> disabled="disabled" </c:if> onclick="checkedFeteA()" value="0" <c:if test="${abroadEdit.fDiningPlace!=1} "> checked </c:if>>否
									</td>
								</tr>
								<tr class="trbody">
									<td class="td1"><span class="style1">*</span>备注</td>
									<td class="td2" colspan="3">
										<input class="easyui-textbox" id="remark" name="remark" data-options="multiline:true,required:true,validType:'length[0,250]'" 
										  style="width:595px;height:60px;" value="${abroadEdit.remark }">
									</td>
								</tr>
								<tr class="trbody">
									<td class="td1" style="width: 70px;"><span class="style1">*</span> 付款方式</td>
									<td class="td2">
										<input class="easyui-combobox" id="paymentTypeShow" value="${bean.paymentType}" required="required" style="width: 230px; height: 30px;" data-options="validType:'selectValid',editable:false,url:base+'/lookup/lookupsJson?parentCode=FKFS&selected=${bean.paymentType}',method:'POST',valueField:'text',textField:'text'"/>
									</td>
								...
								<tr class="trbody" style="line-height: 36px;">
									...
									<td class="td2" >
										<input class="easyui-textbox" id="deptName" name="deptName" readonly="readonly" value="${applyBean.deptName}" style="width: 236px;height: 30px;margin-left: 10px " >
									</td>
								</tr>
						</table>
					</div>				
				...
				<div class="easyui-accordion" style="margin-left: 20px;margin-right: 20px;">
					<div title="出国人员信息" data-options="collapsed:false,collapsible:false"style="overflow:auto;width: 717px">
						<!-- 出国人员信息-->
						<div style="overflow:auto;margin-top: 0px;">
							<jsp:include page="abroad_person.jsp" />
						</div>
					</div>
				</div>
				<div class="easyui-accordion" style="margin-left: 20px;margin-right: 20px">
				...
				<!-- 路线计划 信息 -->
				<div class="easyui-accordion" style="margin-left: 20px;margin-right: 20px;">
					<div title="出访国家（含经停）" data-options="collapsed:false,collapsible:false"style="overflow:auto;width: 717px">
						<!-- 路线计划 -->
						<div style="overflow:auto;margin-top: 0px;">
...
function radiono(){
	...
	$('#fTeamPersonNum').numberbox('readonly',true);
	$('#fThisUnitTeamPersonNum').numberbox('readonly',true);
	$('#abroadDateStart').datebox('readonly',true);
	$('#abroadDateEnd').datebox('readonly',true);
	$('#abroadDay').textbox('readonly',true);
	$('#fTeamLeader').textbox('readonly',true);
	$("#fDiningPlaceId1").attr('disabled','disabled');
	$("#fDiningPlaceId2").attr('disabled','disabled');
	$("#abroad-person-id").hide();
	$('#reimburse_save_form').form('reset');
	$('#abroad-person-dg').datagrid('reload');
	flag=1;
}
function radioyes(){
	$("#abroad-person-id").show();
	$('#radiofupdate').show();
	...
	$('#fTeamPersonNum').numberbox('readonly',false);
	$('#fThisUnitTeamPersonNum').numberbox('readonly',false);
	$('#abroadDateStart').datebox('readonly',false);
	$('#abroadDateEnd').datebox('readonly',false);
...
$(document).ready(function() {
	...
	var	sfyq='${abroadEdit.fDiningPlace}';
	if(sfyq==1){
		$('#feteCostId').show();
...
function checkedFete1(){
	var cxjk =$('input[name="fDiningPlaceChecked"]:checked').val();
	if(cxjk==1){
		$('#feteCostId').show();
		$('#rec-fete-dg-reim').datagrid('reload');
	} else {
		$('#feteCostId').hide();
		var feeRows = $('#rec-fete-dg-reim').datagrid('getRows');
...
function checkedFeteA(){
	var cxjk =$('input[name="fDiningPlaceChecked"]:checked').val();
	if(cxjk==1){
		$('#feteCostId').show();
		$.parser.parse("#feteCostId");
		$('#rec-fete-dg-reim').datagrid('reload');
		$('#fDiningPlace').val(1);
	} else {
		$('#feteCostId').hide();
...
function saveReimburse(flowStauts) {
	...
	//出国人员信息
	var abroadPersonRows = $('#abroad-person-dg').datagrid('getRows');
	var ids = '';
	for (var i = 0 ; i < abroadPersonRows.length ; i++) {
		ids = ids + abroadPersonRows[i].id+',';
	}
	ids = ids.substring(0, ids.length-1);
	$('#fAbroadPeople').val(ids);
	...
	//宴请安排
	$('#fDiningPlace').val($('input[name="fDiningPlaceChecked"]:checked').val());
...
function onSelectAbroad3(date) {
	endday5 = date;
	var startday5 =new Date(startday5);
	if(startday5 !=undefined){
		var d = (endday5 - startday5) / 86400000 + 1;
		if (d > 0) {
			$('#abroadDay').textbox("setValue", d);
