...
<form id="form2">
	<table id="hotelfapiao" class="window-table-readonly-zc" cellspacing="0" cellpadding="0" style=" height: 100px;">
		<c:forEach items="${Invoicelist2 }" varStatus="i" var="invoice">
			<tbody id="hotel_${i.index}" >
				<tr>
					<td class="td1" style="width: 100px;">
						&nbsp;&nbsp;<span id="hotelFp_${i.index}">发票${i.index+1}</span>
						<input type="text" id="hotelfp_${i.index}" value="${invoice.fileId}" name="fileId" hidden="hidden">
						<input type="file" id="hotelimage_${i.index}" onchange="upImageFile2(this)" hidden="hidden">
					</td>
					<td class="td2" colspan="3" id="hotelimagetd_${i.index}" style="width: 150px;">
					...
					<td class="td2" colspan="3" id="">
						<div style="width: auto; height:auto; height:auto;margin-top:5px;margin-left:49px;width: 300px;">
							<p style="margin-bottom:10px;">
								...
								<input class="input_amonut1 a easyui-numberbox" readonly="readonly" name="amount" id="hotelamount_${i.index}" value="${invoice.amount}" onchange="addAmonut2(this)"data-options="precision:2"/>
								<!-- oninput = "value=value.replace(/^\D*(\d*(?:\.\d{0,2})?).*$/g, '$1')" value=""  -->
							</p>
							<p style="margin-bottom:5px;" >
								<span class="hotelfp_span">备注：</span>
								<input  class="input_amonut1 easyui-textbox" readonly="readonly" name="remark" id="hotelremark_${i.index}"value="${invoice.remark}" />
							</p>
						</div>
					</td>
					<td style="vertical-align:top">
						<div>
							<%-- <a onclick="deleteTravel2(this)" style="float: right;" href="#" > 
...
function append2(obj) {
	...
	'<input type="file" id="hotelimage_'+a+'" onchange="upImageFile2(this)" hidden="hidden"></td>'+
	'<td class="td2" colspan="3" id="hotelimagetd_'+a+'" style="width: 150px;">'+
	'<a onclick="clickUpFile2(this)" id="hotelclick_'+a+'" style="font-weight: bold;" href="#">'+
	...
	'<p style="margin-bottom:10px;">'+
	'<span class="hotelfp_span" >金额(元)：</span>'+
	'<input class="input_amonut1 a easyui-numberbox" name="amount" id="hotelamount_'+a+'" onchange="addAmonut2(this)"data-options="precision:2"/>'+
	'</p>'+
	'<p style="margin-bottom:5px;" >'+
	'<span class="hotelfp_span">备注：</span>'+
		...
		$.ajax({
					...
					success : function(data) {
			        	...
			        	if(y!=0){
			        		$('#hotelimagetd_' + j)
							.append(
									'<img style="vertical-align:bottom;margin-left:0px; width: 100px;height: 73px" src="'+data+'" onclick="yl(this.src)"/><a style="color:red" href="#" id="hotelsc_'
											+ j
											+ '" onclick="deleteImage2(this)">    删除</a>');
				        	}else{
				        		$('#hotelimagetd_'+j).append('<img style="vertical-align:bottom;margin-left:0px; width: 100px;height: 73px" src="'+data+'" onclick="yl(this.src)"/><a style="color:red" href="#" id="hotelsc_'+j+'" onclick="deleteImage2(this)">    删除</a>');
				        	}
	...
	var id =obj.id;
     ...
     var y=num.substring(x+1,num.length);
   	  if(y!=0){
   		$('#hotelimagetd_' + j)
		.append(
				'<a onclick="clickUpFile2(this)" id="hotelclick_'
						+ j
     		...
     		$('#hotelimagetd_'+j).append('<a onclick="clickUpFile2(this)" id="hotelclick_'+j+'" style="font-weight: bold;" href="#">'+
