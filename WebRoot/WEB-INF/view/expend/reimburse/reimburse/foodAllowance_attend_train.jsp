...
<div class="window-tab" style="margin-left: 0px;padding-top: 10px;border-top: 1px solid rgba(217,227,231,1);">
	...
	">
		<thead>
			<tr>
				...
				<th data-options="field:'fname',width:220,align:'center',editor:{type:'combobox',options:{editable:false,onHidePanel:onHidePanelName,onShowPanel:onShowPanelName}}" >姓名</th>
	...
	<div id="reimburse_foodtool" style="height:20px;padding-top : 8px">
		...
		<a href="javascript:void(0)" onclick="removeitFoodTraffic()" id="removeitFoodTrafficId" style="float: right;"><img src="${base}/resource-modality/${themenurl}/button/scyh1.png" onMouseOver="mouseOver(this)" onMouseOut="mouseOut(this)"></a>
		<a style="float: right;">&nbsp;&nbsp;</a>
		<a href="javascript:void(0)" onclick="appendFoodTraffic()" id="appendFoodTrafficId" style="float: right;"><img src="${base}/resource-modality/${themenurl}/button/tjyh1.png" onMouseOver="mouseOver(this)" onMouseOut="mouseOut(this)"></a>
	</div>
	</c:if>
</div>
...
function endEditingfood() {
	...
	if ($('#reimbursein_foodtab').datagrid('validateRow', editIndexFood)) {
		...
		var tr = $('#reimbursein_foodtab').datagrid('getEditors', editIndexFood);
...
function onHidePanelName(){
	var index=$('#reimbursein_foodtab').datagrid('getRowIndex',$('#reimbursein_foodtab').datagrid('getSelected'));
	var fnameId = $('#reimbursein_foodtab').datagrid('getEditor',{
		index:index,
		field:'fnameId'
	});
	var fname = $('#reimbursein_foodtab').datagrid('getEditor',{
		index:index,
		field:'fname'
	});
	$(fnameId.target).textbox('setValue', fname.target.combobox('getValues'));
	
}

function onShowPanelName(){
	var index=$('#reimbursein_foodtab').datagrid('getRowIndex',$('#reimbursein_foodtab').datagrid('getSelected'));
	$('#reimbursein_foodtab').datagrid('selectRow', index).datagrid('beginEdit',
			editIndexFood);
