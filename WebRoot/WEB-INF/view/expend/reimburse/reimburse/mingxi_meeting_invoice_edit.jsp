...
<form id="form1">
			...
			<tbody id="meeting_${i.index}" >
				<tr>
					<td class="td1" style="width: 100px;">
						...
						<input type="file" id="meetingImage_${i.index}" accept="image/jpeg,image/jpg,image/png" onchange="upImageFile(this)" hidden="hidden">
					</td>
					<td class="td2" colspan="3" id="meetingZfbimagetd_${i.index}" style="width: 150px;">
						<c:if test="${invoice.fileId!=null&&!empty invoice.fileId}">
							<img style="vertical-align:bottom;width: 100px; height: 73px;margin-left:0px" onclick="yl(this.src)" src="${base}/attachment/download/${invoice.fileId}"/><a style="color:red" href="#" onclick="deleteImage(this)" id="meetingsc_${i.index}">    删除</a>
						</c:if>
						<c:if test="${invoice.fileId==null||empty invoice.fileId}">
							<a onclick="clickUpFile(this)" id="click_${i.index}" style="font-weight: bold;" href="#">
					...
					<td class="td2" colspan="3" id="">
						<div style="width: auto; height:auto; height:auto;margin-top:5px;margin-left:49px;width: 300px;">
							<p style="margin-bottom:10px;">
								...
								<input class="input_amonut1 a easyui-numberbox" name="amount" id="meetingAmount_${i.index}" value="${invoice.amount}" onchange="addAmonut(this)"data-options="precision:2"/>
								<!-- oninput = "value=value.replace(/^\D*(\d*(?:\.\d{0,2})?).*$/g, '$1')" value=""  -->
							</p>
							<p style="margin-bottom:5px;" >
								<span class="fp_span">备注：</span>
								<input  class="input_amonut1 easyui-textbox" name="remark" id="meetingRemark_${i.index}" value="${invoice.remark}" />
							</p>
						</div>
					</td>
					<td style="vertical-align:top">
						<div>
							<c:if test="${i.index!=0 }">
								<a onclick="deleteMeetingEdit(this)" style="float: right;" href="#" > 
	...
	<a onclick="appendMeetingEdit(this)" style="font-weight: bold;vertical-align: middle;margin-left: 308px;" href="#">
...
function countTravel(obj){
	...
	function appendMeetingEdit(obj) {
		index = index + 1;
	...
	function deleteMeetingEdit(item){
