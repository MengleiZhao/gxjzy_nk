...
<form id="form1">
	<table id="outsideAttendfapiao" class="window-table-readonly-zc" cellspacing="0" cellpadding="0" style=" height: 100px;width:695px;">
		<tbody id="outsideAttend_0" >
			<tr>
				<td class="td1" style="width: 100px;">
					&nbsp;&nbsp;<span id="outsideAttendFp_0">发票1</span>
					<input type="text" id="outsideAttendfp_0" name="fileId" hidden="hidden">
					<input type="file" id="outsideAttendImage_0" accept="image/jpeg,image/jpg,image/png" onchange="upImageFileoutsideAttend(this)" hidden="hidden">
				</td>
				<td class="td2" colspan="3" id="outsideAttendZfbimagetd_0" style="width: 150px;">
					<a onclick="clickUpFileoutsideAttend(this)" id="outsideAttendClick_0" style="font-weight: bold;" href="#">
				...
				<td class="td2" colspan="3" id="">
					<div style="width: auto; height:auto; height:auto;margin-top:5px;margin-left:49px;width: 300px;">
						<p style="margin-bottom:10px;">
							<span class="outsideAttendfp_span" >金额(元)：</span>
							<input class="input_amonut1 a easyui-numberbox" name="amount" id="outsideAttendAmount_0" onchange="addAmonut(this)"data-options="precision:2"/>
						</p>
						<p style="margin-bottom:5px;" >
							<span class="outsideAttendfp_span">备注：</span>
							<input  class="input_amonut1 easyui-textbox" name="remark" id="outsideAttendRemark_0" value="" />
						</p>
					</div>
				</td>
			</tr>
	...
	<a onclick="appendoutsideAttendAdd(this)" style="font-weight: bold;vertical-align: middle;margin-left: 308px;" href="#">
...
function countDays(obj){
	...
	if (d > 0) {
		var amount =parseFloat($('#outsideAttendAmount_' + i).val());
		if(!isNaN(amount)){
...
function countTravel(obj){
	...
	var i=id.substring(j+1,id.length);
	var num =0;
	var num1 =parseFloat($('#outsideAttendAmount_' + i).val());
	...
	function appendoutsideAttendAdd(obj) {
		index = index + 1;
		var fromNum = parseInt(obj.parentNode.parentNode.children[5].children[0].children.length);
		$('#outsideAttendfapiao').append('<tbody id="outsideAttend_'+a+'" >'+
		'<tr><td class="td1" style="width: 100px;">&nbsp;&nbsp;<span id="outsideAttendFp_'+a+'">发票'+(fromNum+1)+'</span>'+
		'<input type="text" id="outsideAttendfp_'+a+'" name="fileId" hidden="hidden">'+
		'<input type="file" id="outsideAttendImage_'+a+'" accept="image/jpeg,image/jpg,image/png" onchange="upImageFileoutsideAttend(this)" hidden="hidden"></td>'+
		'<td class="td2" colspan="3" id="outsideAttendZfbimagetd_'+a+'" style="width: 150px;">'+
		'<a onclick="clickUpFileoutsideAttend(this)" id="outsideAttendClick_'+a+'" style="font-weight: bold;" href="#">'+
		'<img style="vertical-align:bottom;margin-left:30px" src="${base}/resource-modality/${themenurl}/button/shangchuan1.png" onMouseOver="mouseOver(this)" onMouseOut="mouseOut(this)">'+
		'</a></td>'+
		'<td class="td2" colspan="3" id="">'+
		'<div style="width: auto; height:auto; height:auto;margin-top:5px;margin-left:49px;width: 300px;">'+
		'<p style="margin-bottom:10px;">'+
		'<span class="outsideAttendfp_span" >金额(元)：</span>'+
		'<input class="input_amonut1 a easyui-numberbox" name="amount" id="outsideAttendAmount_'+a+'" onchange="addAmonut(this)"data-options="precision:2"/>'+
		'</p>'+
		'<p style="margin-bottom:5px;" >'+
		'<span class="outsideAttendfp_span">备注：</span>'+
		'<input  class="input_amonut1 easyui-textbox" name="remark" id="outsideAttendRemark_'+a+'" value="" />'+
		'</p></div>'+
		'</td>'+
		'<td style="vertical-align:top">'+
		'<div>'+
		'<a onclick="deleteoutsideAttend(this)" style="float: right;" href="#" > '+
		...
		'</td>'+
		'</tr></tbody>');
		$.parser.parse($('#outsideAttend_'+a).parent());
	...
	function clickUpFileoutsideAttend(obj) {
		
		var id = obj.id;
		var i = id.lastIndexOf("\_");
		var j = id.substring(i + 1, id.length);
		$('#outsideAttendImage_' + j).click();
	}

	//上传图片文件转化为base64在页面显示
	function upImageFileoutsideAttend(obj) {
		var id = obj.id;
		var i = id.lastIndexOf("\_");
		var j = id.substring(i + 1, id.length);
		if (obj == null || obj == "") {
		...
		for (var i = 0; i < files.length; i++) {
			...
			$.ajax({
						...
						success : function(data) {
							upladFp(obj, "outsideAttend", "fp04");
				        	var num = obj.parentNode.parentNode.parentNode.id;
				        	$('#outsideAttendZfbimagetd_'+j).empty();
				        	$('#outsideAttendImage_'+j).val("");
				        	var x=num.lastIndexOf("\_");
				   	     	var y=num.substring(x+1,num.length);
				        	if(y!=0){
				        		$('#outsideAttendZfbimagetd_' + j)
								.append(
										'<img style="vertical-align:bottom;margin-left:0px; width: 100px;height: 73px" src="'+data+'" onclick="yl(this.src)"/><a style="color:red" href="#" id="sc_'
												+ j
												+ '" onclick="deleteImageoutsideAttend(this)">    删除</a>');
					        	}else{
					        		$('#outsideAttendZfbimagetd_'+j).append('<img style="vertical-align:bottom;margin-left:0px; width: 100px;height: 73px" src="'+data+'" onclick="yl(this.src)"/><a style="color:red" href="#" id="sc_'+j+'" onclick="deleteImageoutsideAttend(this)">    删除</a>');
					        	}
	...
	//删除上传的图片
	function deleteImageoutsideAttend(obj) {
		var id =obj.id;
	     ...
	     var y=num.substring(x+1,num.length);
	   	  if(y!=0){
	   		$('#outsideAttendZfbimagetd_' + j)
			.append(
					'<a onclick="clickUpFileoutsideAttend(this)" id="outsideAttendClick_'
							+ j
      		...
      		$('#outsideAttendZfbimagetd_'+j).append('<a onclick="clickUpFileoutsideAttend(this)" id="outsideAttendClick_'+j+'" style="font-weight: bold;" href="#">'+
	...
	function  deleteoutsideAttend(item){
		...
		for(var i=1;i<fromNum;i++){
			if('outsideAttend'==arr[0]){
				var tableId = $('#outsideAttendfapiao').find('tbody');
