...
<%@ include file="/includes/taglibs.jsp"%>
			...
			<div title="预算信息" data-options="collapsed:false,collapsible:false"style="overflow:auto;margin-left: 0px;">
				<jsp:include page="apply_index_multiple_detail.jsp" />
			</div>
			</div>
			<div class="easyui-accordion" style="margin-left: 20px;margin-right: 20px">
			<div title="基本信息" data-options="collapsed:false,collapsible:false" style="overflow:auto;width: 717px">
				<table class="window-table" style="margin-top: 3px;" cellspacing="0" cellpadding="0">
						<tr class="trbody">
							...
							<td colspan="3">
								<input class="easyui-textbox" style="width: 625px;height: 30px;margin-left: 10px" value="${bean.gName}" readonly="readonly" required="required" data-options="validType:'length[1,50]'"/>
							</td>
						</tr>
						<tr class="trbody">
							<td class="td1" style="width: 70px;"><span class="style1">*</span>培训名称</td>
							<td colspan="3">
								<input class="easyui-textbox" style="width: 625px;height: 30px;margin-left: 10px" value="${bean.reimName}" readonly="readonly" required="required" data-options="validType:'length[1,100]'"/>
							</td>
						</tr>
						<tr class="trbody" style="line-height: 65px;">
							<td class="td1" style="width: 70px;"><span class="style1">*</span>培训事由</td>
							<td colspan="3">
								<textarea name="reimburseReasons" id="reimburseReasons" class="easyui-textbox" data-options="multiline:true" readonly="readonly"
						...
						<tr class="trbody">
							...
							<td class="td2">
								<input  class="easyui-datebox" style="width: 260px;; height: 30px;" id="travelDateStart"  readonly="readonly"
							...
							<td class="td2">
								<input class="easyui-datebox" style="width: 262px;; height: 30px;" id="travelDateEnd"  readonly="readonly"
						...
						<tr class="trbody">
							...
							<td class="td2">
								<input  class="easyui-combobox" style="width: 260px;; height: 30px;" id="fProvinceId"  readonly="readonly"
							...
							<td class="td2">
								<input class="easyui-combobox" style="width: 262px;; height: 30px;" id="fCityId"  readonly="readonly"
								value="${reimTravelBean.fCityId}" required="required" editable="false" data-options="editable:false,
									method:'POST',
						...
						<tr class="trbody">
							...
							<td class="td2">
								<input  class="easyui-combobox" style="width: 260px;; height: 30px;" id="fDistrictId"  readonly="readonly"
								value="${reimTravelBean.fDistrictId}" required="required" editable="false" data-options="editable:false,
									url:'${base}/apply/getRegion?selected=${reimTravelBean.fDistrictId}',method:'POST',valueField:'id',textField:'text'"/>
							</td>
							<td class="td1" style="width: 70px;"><span class="style1">*</span> 详细地址</td>
							<td class="td2">
								<input class="easyui-textbox" style="width: 262px;; height: 30px;" id="travelAreaName"  readonly="readonly"
						...
						<tr class="trbody">
							...
							<td colspan="3">
								<input class="easyui-textbox" style="width: 625px;height: 30px;margin-left: 10px" id="travelAttendPeop" readonly="readonly" value="${reimTravelBean.travelAttendPeop}" required="required"
								 data-options="validType:'length[1,200]',editable:false,icons:[{iconCls:'icon-add'}]"/>
							</td>
						</tr>
						</table>
					</div>				
			...
			<div class="easyui-accordion" style="margin-left: 20px;margin-right: 20px;margin-top: 40px">
				<div title="费用明细" data-options="collapsed:false,collapsible:false" style="overflow:auto;">
					...
					<div style="overflow:auto;">
						<jsp:include page="apply_outside_traffic_attend_train_detail.jsp" />
					</div>
					<div style="overflow:hidden;margin-top: 0px">
						<!-- 城市间交通费发票明细 -->
						<jsp:include page="mingxi_travel_outside_detail_attend_train.jsp" />
					</div>
					<!-- 市内交通费 -->
					<div style="overflow:auto;margin-top: 20px;">
						<jsp:include page="apply_in_city_attend_train_detail.jsp" />
					</div>
					<!-- 住宿费 -->
					<div style="overflow:auto;margin-top: 20px;">
						<jsp:include page="hotelExpense_attend_train_detail.jsp" />
					</div>
					<div style="overflow:hidden;margin-top: 0px">
						<!-- 住宿费发票明细 -->
						<jsp:include page="mingxi_travel_hotel_detail_attend_train.jsp" />
					</div>
					<!-- 伙食费 -->
					<div style="overflow:auto;margin-top: 20px;">
						<jsp:include page="foodAllowance_attend_train_detail.jsp" />
					</div>
					<div>
			...
			<div title="附件信息" data-options="collapsed:false,collapsible:false"
				...
				<table class="window-table" cellspacing="0" cellpadding="0" style="margin-top: 3px;">
					<tr>
						<td style="width:75px;text-align: left">附件</td>
						<td colspan="3" id="tdf">
							&nbsp;&nbsp;
...
function allProIndexList(){
	...
	$('#travelAmount').val((parseFloat(foodAmount)+parseFloat(hotelAmount)+parseFloat(cityAmount)+parseFloat(outsideAmount)+parseFloat(trainMoney)).toFixed(2));
	$("#rapplyTotalAmount").html((parseFloat(foodAmount)+parseFloat(hotelAmount)+parseFloat(cityAmount)+parseFloat(outsideAmount)+parseFloat(meetTrainAmount)+parseFloat(trainMoney)).toFixed(2)+"元");
	$("#p_amount").html((parseFloat(foodAmount)+parseFloat(hotelAmount)+parseFloat(cityAmount)+parseFloat(outsideAmount)+parseFloat(meetTrainAmount)+parseFloat(trainMoney)).toFixed(2)+"元");
	$("#reimburseAmount").val((parseFloat(foodAmount)+parseFloat(hotelAmount)+parseFloat(cityAmount)+parseFloat(outsideAmount)+parseFloat(meetTrainAmount)+parseFloat(trainMoney)).toFixed(2));
}
...
$('#fProvinceId').combobox({
	onSelect: function(date){
		...
		$("#fProvinceIds").val(date.id);
	}
});
$('#fCityId').combobox({
	onSelect: function(date){
		
		$("#fCityIds").val(date.id);
	}
});
