		...
		selectOnCheck: true,checkOnSelect:true,remoteSort:true,nowrap:false,striped: true,fitColumns: true" >
			<thead>
				<tr>
					...
					<th data-options="field:'gCode',align:'center',resizable:false" style="width: 15%">报销单编号</th>
					<th data-options="field:'gName',align:'center',resizable:false,formatter:formatGName" style="width: 15%">报销摘要名称</th>
					<th data-options="field:'amount',align:'center',formatter:listToFixed" style="width: 15%">申请金额(元)</th>
					<th data-options="field:'deptName',align:'center',resizable:false" style="width: 15%">报销部门</th>
					<th data-options="field:'userNames',align:'center',resizable:false" style="width: 15%">报销人</th>
					...
					<th data-options="field:'reimbTime',align:'center',resizable:false,formatter: ChangeDateFormat" style="width: 10%">报销日期</th>
...
function formatGName(val, row){
	if(row.reimbFlowStauts==null){
		return val;
	}else{
		return row.rName;
	}
...
function Baoxiaochaoe(val, row){
	var a = row.amount;
	var b = row.reimbAmount;
	if('${reimburseType}'=='4'){
		return fomatMoney(val,2);
	}
	
	if('${reimburseType}'=='12'){
		if(parseFloat(row.meetTrainAmount)+parseFloat(row.hotelAmount)>row.amount){
...
function addReimburse(gId,reimburseType) {
	...
	if(reimburseType=="1") {
		...
		title="公务出国报销";
	} else if(reimburseType=="12") {
		title="参加培训报销";
	} else if(reimburseType=="13") {
		title="市内公务非公共交通报销";
	}
	/* if(reimburseType=="1"){
