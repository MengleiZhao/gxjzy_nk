...
<div class="window-tab" style="margin-left: 0px;padding-top: 10px;">
	...
	singleSelect: true,
	<c:if test="${operation=='add'}">
	onLoadSuccess:onLoadSuccessLessons
	</c:if>
	">
	<thead>
		<tr>
				<th data-options="field:'thId',hidden:true"></th>
				<th data-options="field:'lecturerName',required:'required',align:'center',width:135">讲师姓名</th>
				<th data-options="field:'fIdentityNumber',align:'center',hidden:true">身份证号</th>
				<th data-options="field:'lessonTime',required:'required',width:110,align:'center'">学时</th>
				<th data-options="field:'lessonStd',required:'required',align:'center',width:130">讲课费标准（元/学时）</th>
				...
				<c:if test="${empty detail}">
				<th data-options="field:'fNetAmount',required:'required',align:'center',width:130,editor:{type:'numberbox',options:{precision:2,editable: false}}">税后金额[元]</th>
				</c:if>
				<c:if test="${!empty detail}">
				<th data-options="field:'fNetAmount',required:'required',align:'center',width:130,editor:{type:'numberbox',options:{precision:2,groupSeparator:',',editable: false}}">税后金额[元]</th>
				</c:if>
				<th data-options="field:'isOutside',hidden:true"></th>
				<c:if test="${empty detail}">
				<th data-options="field:'fIndividualIncomeTax',required:'required',align:'center',width:130,editor:{type:'numberbox',options:{precision:2,editable: false,onChange:individualIncomeTaxOnChange}}">预扣预缴税款[元]</th>
				</c:if>
				<c:if test="${!empty detail}">
				<th data-options="field:'fIndividualIncomeTax',required:'required',align:'center',width:130,editor:{type:'numberbox',options:{precision:2,groupSeparator:',',editable: false}}">预扣预缴税款[元]</th>
				</c:if>
				<c:if test="${empty detail}">
				<th data-options="field:'reimbSum',required:'required',align:'center',width:130,editor:{type:'numberbox',options:{onChange:addNum2,precision:2,groupSeparator:',',editable: false}}">报销金额[元]</th>
				</c:if>
				<c:if test="${!empty detail}">
				<th data-options="field:'reimbSum',required:'required',align:'center',width:130,editor:{type:'numberbox',options:{precision:2,groupSeparator:',',editable: false}}">报销金额[元]</th>
				</c:if>
		</tr>
	</thead>
	</table>
...
//计算申请金额
function individualIncomeTaxOnChange(newValue,oldValue) {
		var index=$('#mingxi-lessons-dg').datagrid('getRowIndex',$('#mingxi-lessons-dg').datagrid('getSelected'));//获得选中行的行号
		var num = 0;
		var rows = $('#mingxi-lessons-dg').datagrid('getRows');
		for(var i=0;i<rows.length;i++){
			if(i!=index){
				if(rows[i].fIndividualIncomeTax!=""&&rows[i].fIndividualIncomeTax!=null){
					num += parseFloat(rows[i].fIndividualIncomeTax);
				}
			}
		}
		if(newValue!=""&&newValue!=null) {
		...
		$('#fIndividualIncomeTax').val(num.toFixed(2));
		$('#fIndividualIncomeTaxs').val(fomatMoney(num,2));
}
//计算申请金额
function addNumss(newValue,oldValue) {
		...
		if(periodStd>parseFloat(standar)){
			...
			tr[1].target.numberbox('setValue',0);		//申请金额
			newValue=0;
		}else{
			var countMoney = fIndividualIncomeTax+netAmount;
			tr[1].target.numberbox('setValue',netAmount.toFixed(2));
			tr[2].target.numberbox('setValue',fIndividualIncomeTax);
			tr[3].target.numberbox('setValue',countMoney);
		}
}

function onLoadSuccessLessons(){
	var rows = $('#mingxi-lessons-dg').datagrid('getRows');
	var fIndividualIncomeTaxList = 0.00;
	for(var i=0;i<rows.length;i++){
		var applySum = isNaN(parseFloat(rows[i].applySum))?0:parseFloat(rows[i].applySum);//获得该行的申请金额
		var fIndividualIncomeTax = isNaN(parseFloat(rows[i].fIndividualIncomeTax))?0:parseFloat(rows[i].fIndividualIncomeTax);//个人所得税
		...
		fIndividualIncomeTaxList+=fIndividualIncomeTax;
	}
	$('#fIndividualIncomeTaxs').val(fIndividualIncomeTaxList);
	$('#fIndividualIncomeTax').val(fIndividualIncomeTaxList);
}
</script>
