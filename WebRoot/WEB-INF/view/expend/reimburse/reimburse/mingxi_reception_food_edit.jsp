...
<form id="formFood">
	<table id="foodfapiao" class="window-table-readonly-zc" cellspacing="0" cellpadding="0" style=" height: 100px;width:695px;">
		<c:forEach items="${InvoicelistFood }" varStatus="i" var="invoice">
			<tbody id="food_${i.index}" >
				<tr>
					<td class="td1" style="width: 100px;">
						&nbsp;&nbsp;<span id="foodFp_${i.index}">发票${i.index+1}</span>
						<input type="text" id="foodfp_${i.index}" value="${invoice.fileId}" name="fileId" hidden="hidden">
						<input type="file" id="foodimage_${i.index}" accept="image/jpeg,image/jpg,image/png" onchange="upImageFileFood(this)" hidden="hidden">
					</td>
					<td class="td2" colspan="3" id="foodimagetd_${i.index}" style="width: 150px;">
						<c:if test="${invoice.fileId!=null&&!empty invoice.fileId}">
							<img style="vertical-align:bottom;width: 100px; height: 73px;margin-left:0px" onclick="yl(this.src)" src="${base}/attachment/download/${invoice.fileId}"/><a style="color:red" href="#" onclick="deleteImageFood(this)" id="foodsc_${i.index}">    删除</a>
						</c:if>
						<c:if test="${invoice.fileId==null||empty invoice.fileId}">
							<a onclick="clickUpFileFood(this)" id="foodclick_${i.index}" style="font-weight: bold;" href="#">
					...
					<td class="td2" colspan="3" id="">
						<div style="width: auto; height:auto; height:auto;margin-top:5px;margin-left:49px;width: 300px;">
							<p style="margin-bottom:10px;">
								...
								<input class="input_amonut1 a easyui-numberbox" name="amount" id="foodamount_${i.index}" value="${invoice.amount}" onchange="addAmonut2(this)"data-options="precision:2"/>
								<!-- oninput = "value=value.replace(/^\D*(\d*(?:\.\d{0,2})?).*$/g, '$1')" value=""  -->
							</p>
							<p style="margin-bottom:5px;" >
								<span class="foodfp_span">备注：</span>
								<input  class="input_amonut1 easyui-textbox" name="remark" id="foodremark_${i.index}"value="${invoice.remark}" />
							</p>
						</div>
					</td>
	...
	<a onclick="append2(this)" style="font-weight: bold;vertical-align: middle;margin-left: 308px;" href="#">
	...
	for (var i = 0; i < files.length; i++) {
		...
		$.ajax({
					...
					success : function(data) {
						upladFp(obj, "receptionfood", "fp04");
			        	...
			        	if(y!=0){
			        		$('#foodimagetd_' + j)
							.append(
									'<img style="vertical-align:bottom;margin-left:0px; width: 100px;height: 73px" src="'+data+'" onclick="yl(this.src)"/><a style="color:red" href="#" id="foodsc_'
											+ j
											+ '" onclick="deleteImageFood(this)">    删除</a>');
				        	}else{
				        		$('#foodimagetd_'+j).append('<img style="vertical-align:bottom;margin-left:0px; width: 100px;height: 73px" src="'+data+'" onclick="yl(this.src)"/><a style="color:red" href="#" id="foodsc_'+j+'" onclick="deleteImageFood(this)">    删除</a>');
				        	}
