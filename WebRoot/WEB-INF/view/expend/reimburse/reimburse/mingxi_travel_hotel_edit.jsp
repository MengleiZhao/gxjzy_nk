...
<form id="form2">
			...
			<tbody id="hotel_${i.index}" >
				<tr>
					<td class="td1" style="width: 100px;">
						...
						<input type="file" id="hotelimage_${i.index}" accept="image/jpeg,image/jpg,image/png" onchange="upImageFileHotel(this)" hidden="hidden">
					</td>
					<td class="td2" colspan="3" id="hotelimagetd_${i.index}" style="width: 150px;">
						<c:if test="${invoice.fileId!=null&&!empty invoice.fileId}">
							...
							<a style="color:red" href="#" onclick="deleteImageHotel(this)" id="hotelsc_${i.index}">    删除</a>
						</c:if>
						<c:if test="${invoice.fileId==null||empty invoice.fileId}">
						<a onclick="clickUpFile2(this)" id="hotelclick_${i.index}" style="font-weight: bold;" href="#">
					...
					<td class="td2" colspan="3" id="">
						<div style="width: auto; height:auto; height:auto;margin-top:5px;margin-left:49px;width: 300px;">
							<p style="margin-bottom:10px;">
								...
								<input class="input_amonut1 a easyui-numberbox" name="amount" id="hotelamount_${i.index}" value="${invoice.amount}" onchange="addAmonut2(this)"data-options="precision:2"/>
								<!-- oninput = "value=value.replace(/^\D*(\d*(?:\.\d{0,2})?).*$/g, '$1')" value=""  -->
							</p>
							<p style="margin-bottom:5px;" >
								<span class="hotelfp_span">备注：</span>
								<input  class="input_amonut1 easyui-textbox" name="remark" id="hotelremark_${i.index}"value="${invoice.remark}" />
							</p>
						</div>
					</td>
	...
	<a onclick="appendHotelEditTravel(this)" style="font-weight: bold;vertical-align: middle;margin-left: 308px;" href="#">
...
function appendHotelEditTravel(obj) {
	index = index + 1;
					...
					success : function(data) {
			        	...
			        	if(y!=0){
							...
							.append(
				        		...
				        		$('#hotelimagetd_'+j).append('<img style="vertical-align:bottom;margin-left:0px; width: 100px;height: 73px" src="'+data+'" onclick="yl(this.src)"/><a style="color:red" href="#" id="hotelsc_'+j+'" onclick="deleteImageHotel(this)">    删除</a>');
				        	}
