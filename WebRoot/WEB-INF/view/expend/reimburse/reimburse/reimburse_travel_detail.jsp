		...
		<div class="window-left-div" style="width:755px;height: 491px;border: 1px solid #D9E3E7;margin-top: 20px">
			...
			<form id="reimburse_save_form" method="post"  enctype="multipart/form-data">
				...
				<div id="panelID" class="easyui-panel" data-options="closed:true">
					...
					<input type="hidden" id="cxAmounts" name="cxAmount" value="${bean.cxAmount}" />
				</div>
				...
				<!-- 各项明细金额 -->
				<input type="hidden" id="outsideAmount" value="${applyBean.outsideAmount}" />
				<input type="hidden" id="cityAmount" value="${applyBean.cityAmount}" />
				<input type="hidden" id="hotelAmount" value="${applyBean.hotelAmount}" />
				<input type="hidden" id="foodAmount" value="${applyBean.foodAmount}" />
			</form>	
				<div class="tab-wrapper" id="reimburse-travelinfo-add">
					...
					<div class="tab-content">
						<div title="报销" style="margin-bottom:35px;" data-options="iconCls:'icon-xxlb'">
							<jsp:include page="reimburset_travelinfo_detail.jsp" />
						</div>
						<div title="申请" style="overflow:auto;margin-bottom:35px;" data-options="iconCls:'icon-xxlb'">
...
function onclickreimburset(){
	if(itineraryurlcount>0){
		...
		$('#reimburse_itinerary_detail_tab_id').datagrid('reload');
...
function saveReimburse(flowStauts) {
	/* $('#hskAccount').val($('#skAccount').numberbox('getValue'));//转账金额
	$('#hbank').val($('#bank').textbox('getValue'));//开户行
	$('#hbankAccount').val($('#bankAccount').textbox('getValue'));//银行账号
	$('#hidCard').val($('#idCard').textbox('getValue'));//身份证
	$('#hpayeeName').val($('#payeeName').textbox('getValue'));//收款账号
	...
	for(var i=0;i<rowsHotel.length;i++){
		...
		if((rowsHotel[i].applyAmount).toString()==''){
			alert("请填写住宿费的申请金额！");
			return false;
		}
	}
