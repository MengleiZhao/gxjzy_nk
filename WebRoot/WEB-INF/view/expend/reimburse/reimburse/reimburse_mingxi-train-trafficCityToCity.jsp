...
<div class="window-tab" style="margin-left: 0px;padding-top: 10px">
	...
	<div id="trafficCityToCity_Id" style="height:30px;padding-top : 8px">
		...
		<a style="float: left;color: #666666;font-size:12px;">申请金额：<span ><fmt:formatNumber groupingUsed="true" value="${trainingBean.lessonsOutMoney}" minFractionDigits="2" maxFractionDigits="2"/>[元]</span></a>
		<a style="float: left;">&nbsp;&nbsp;</a>
		<c:if test="${operation=='add'}">
		<a style="float: left;color: #666666;font-size:12px;">汇总金额：<span id="traffic1Amount"><fmt:formatNumber groupingUsed="true" value="${reimbTrainingBean.lessonsOutMoney}" minFractionDigits="2" maxFractionDigits="2"/>[元]</span></a>
		</c:if>
		<c:if test="${operation!='add'}">
		<a style="float: left;color: #666666;font-size:12px;">汇总金额：<span id="traffic1Amount"><fmt:formatNumber groupingUsed="true" value="${reimbTrainingBean.lessonsOutMoney}" minFractionDigits="2" maxFractionDigits="2"/>[元]</span></a>
		</c:if>
	...
	<thead>
		<tr>
			...
			<th data-options="field:'applySum',hidden:true"></th>
				...
				<th data-options="field:'vehicle',width:170,align:'center',editor:{
							editable:true,
							type:'combobox',
							options:{
								required:true,
...
function onLoadSuccessToCity(){
	...
	for(var i=0;i<rows.length;i++){
		...
		$('#mingxi-trafficCityToCity-dg').datagrid('updateRow',{
			index: i,
			row: {
				reimbSum: applySum
			}
