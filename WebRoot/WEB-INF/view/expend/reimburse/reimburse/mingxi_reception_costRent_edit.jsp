...
<form id="formCostRent">
	<table id="costRentfapiao" class="window-table-readonly-zc" cellspacing="0" cellpadding="0" style=" height: 100px;width:695px;">
		<c:forEach items="${costRentList }" varStatus="i" var="invoice">
			<tbody id="costRent_${i.index}" >
				<tr>
					<td class="td1" style="width: 100px;">
						&nbsp;&nbsp;<span id="costRentFp_${i.index}">发票${i.index+1}</span>
						<input type="text" id="costRentfp_${i.index}" value="${invoice.fileId}" name="fileId" hidden="hidden">
						<input type="file" id="costRentImage_${i.index}" accept="image/jpeg,image/jpg,image/png" onchange="upImageFileCostRent(this)" hidden="hidden">
					</td>
					<td class="td2" colspan="3" id="costRentZfbimagetd_${i.index}" style="width: 150px;">
						<c:if test="${invoice.fileId!=null&&!empty invoice.fileId}">
							<img style="vertical-align:bottom;width: 100px; height: 73px;margin-left:0px" onclick="yl(this.src)" src="${base}/attachment/download/${invoice.fileId}"/><a style="color:red" href="#" onclick="deleteImageCostRent(this)" id="costRentsc_${i.index}">    删除</a>
						</c:if>
						<c:if test="${invoice.fileId==null||empty invoice.fileId}">
							<a onclick="clickUpFileCostRent(this)" id="clickCostRent_${i.index}" style="font-weight: bold;" href="#">
					...
					<td class="td2" colspan="3" id="">
						<div style="width: auto; height:auto; height:auto;margin-top:5px;margin-left:49px;width: 300px;">
							<p style="margin-bottom:10px;">
								...
								<input class="input_amonut1 a easyui-numberbox" name="amount" id="amount_${i.index}" value="${invoice.amount}" onchange="addAmonut(this)"data-options="precision:2"/>
								<!-- oninput = "value=value.replace(/^\D*(\d*(?:\.\d{0,2})?).*$/g, '$1')" value=""  -->
							</p>
							<p style="margin-bottom:5px;" >
								<span class="fp_span">备注：</span>
								<input  class="input_amonut1 easyui-textbox" name="remark" id="remark_${i.index}" value="${invoice.remark}" />
							</p>
						</div>
					</td>
					<td style="vertical-align:top">
						<div>
							<a onclick="deleteCostRent(this)" style="float: right;" href="#" > 
	...
	<a onclick="append(this)" style="font-weight: bold;vertical-align: middle;margin-left: 308px;" href="#">
...
function countTravel(obj){
	...
	function append(obj) {
		...
		$('#costRentfapiao').append('<tbody id="costRent_'+a+'" >'+
		'<tr><td class="td1" style="width: 100px;">&nbsp;&nbsp;<span id="costRentFp_'+a+'">发票'+(fromNum+1)+'</span>'+
		'<input type="text" id="costRentfp_'+a+'" name="fileId" hidden="hidden">'+
		'<input type="file" id="costRentImage_'+a+'" accept="image/jpeg,image/jpg,image/png" onchange="upImageFileCostRent(this)" hidden="hidden"></td>'+
		'<td class="td2" colspan="3" id="costRentZfbimagetd_'+a+'" style="width: 150px;">'+
		'<a onclick="clickUpFileCostRent(this)" id="clickCostRent_'+a+'" style="font-weight: bold;" href="#">'+
		'<img style="vertical-align:bottom;margin-left:30px" src="${base}/resource-modality/${themenurl}/button/shangchuan1.png" onMouseOver="mouseOver(this)" onMouseOut="mouseOut(this)">'+
		'</a></td>'+
		'<td class="td2" colspan="3" id="">'+
		'<div style="width: auto; height:auto; height:auto;margin-top:5px;margin-left:49px;width: 300px;">'+
		...
		'</td>'+
		'</tr></tbody>');
		$.parser.parse($('#costRent_'+a).parent());
	...
	function clickUpFileCostRent(obj) {
		var id = obj.id;
		var i = id.lastIndexOf("\_");
		var j = id.substring(i + 1, id.length);
		$('#costRentImage_' + j).click();
	}

	//上传图片文件转化为base64在页面显示
	function upImageFileCostRent(obj) {
		var id = obj.id;
		var i = id.lastIndexOf("\_");
		var j = id.substring(i + 1, id.length);
		if (obj == null || obj == "") {
		...
		for (var i = 0; i < files.length; i++) {
			...
			$.ajax({
						...
						success : function(data) {
							upladFp(obj, "costRent", "fp04");
				        	var num = obj.parentNode.parentNode.parentNode.id;
				        	$('#costRentZfbimagetd_'+j).empty();
				        	$('#costRentImage_'+j).val("");
				        	var x=num.lastIndexOf("\_");
				   	     	var y=num.substring(x+1,num.length);
				        	if(y!=0){
				        		$('#costRentZfbimagetd_' + j)
								.append(
										'<img style="vertical-align:bottom;margin-left:0px; width: 100px;height: 73px" src="'+data+'" onclick="yl(this.src)"/><a style="color:red" href="#" id="costRentsc_'
												+ j
												+ '" onclick="deleteImageCostRent(this)">    删除</a>');
					        	}else{
					        		$('#costRentZfbimagetd_'+j).append('<img style="vertical-align:bottom;margin-left:0px; width: 100px;height: 73px" src="'+data+'" onclick="yl(this.src)"/><a style="color:red" href="#" id="costRentsc_'+j+'" onclick="deleteImageCostRent(this)">    删除</a>');
					        	}
	...
	//删除上传的图片
	function deleteImageCostRent(obj) {
		var id =obj.id;
		 var i=id.lastIndexOf("\_");
	     var j=id.substring(i+1,id.length);
	     var num = obj.parentNode.parentNode.parentNode.id;
	     ...
	     var y=num.substring(x+1,num.length);
	   	  if(y!=0){
	   		$('#costRentZfbimagetd_' + j)
			.append(
					'<a onclick="clickUpFileCostRent(this)" id="clickCostRent_'
							+ j
      		...
      		$('#costRentZfbimagetd_'+j).append('<a onclick="clickUpFileCostRent(this)" id="clickCostRent_'+j+'" style="font-weight: bold;" href="#">'+
	...
	function  deleteCostRent(item){
		...
		for(var i=1;i<fromNum;i++){
			if('costRent'==arr[0]){
				var tableId = $('#costRentfapiao').find('tbody');
