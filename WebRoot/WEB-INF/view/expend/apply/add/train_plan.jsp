...
<div class="window-tab" style="margin-left: 0px;padding-top: 10px">
	...
	<div id="rph" hidden="hidden" style="height:30px;padding-top : 8px">
	...
	">
		<thead>
			<tr>
				...
				<th data-options="field:'timeStart',width:130,align:'center',editor:{type:'datebox',options:{showSeconds:false,onHidePanel:changeTime1,editable:false}},formatter:ChangeDateFormatTrain">培训日期</th>
				<th data-options="field:'startHourMinute',width:130,align:'center',editor:{type:'timespinner',options:{showSeconds:false,editable:true,onChange:onChangeStartTimes}}">起始时间</th>
				<th data-options="field:'endHourMinute',width:130,align:'center',editor:{type:'timespinner',options:{showSeconds:false,editable:true,onChange:onChangeEndTimes}}">结束时间</th>
				<th data-options="field:'arrange',width:205,align:'center',editor:'textbox'">培训内容</th>
				<th data-options="field:'lecturerName',width:135,align:'center',editor:{type:'combobox',options:{
                                valueField:'id',
                                textField:'text',
                                multiple: true,
				...
				<th data-options="field:'lessonTime',width:80,align:'center',editor:{type:'numberbox',options:{onChange:onChangeLessonTimes}}">学时</th>
...
function savePlan(){
	...
	for (var i = rows.length-1; i >= 0; i--) {
		if(rows[i].timeStart==''&&rows[i].timeEnd==''&&rows[i].lecturerName==''&&rows[i].lessonTime==''&&rows[i].arrange==''){
			$('#dg_train_plan').datagrid('deleteRow',i);
		}
	}
...
//接待人员表格添加删除，保存方法
	...
	function changeTime1(){
		...
		var flag = lecturerHoursArrs(index,lessonTime,timeStarts,lecturerNumbers);
		if(!flag){
...
function onChangeStartTimes(newVal,oldVal){
	...
	var lessonTime = dateTimeDifference(newVal, $(endHourMinute.target).timespinner('getValue'), "HH");
	if(lessonTime>8){
...
function onChangeEndTimes(newVal,oldVal){
	if(newVal==undefined || oldVal==undefined){
		return false;
	}
	var index=$('#dg_train_plan').datagrid('getRowIndex',$('#dg_train_plan').datagrid('getSelected'));
	...
	var lessonTime = dateTimeDifference($(startHourMinute.target).timespinner('getValue'),newVal, "HH");
	if(lessonTime>8){
		$(lessonTimes.target).textbox('setValue','8');
	}else{
		$(lessonTimes.target).textbox('setValue',lessonTime);
...
/* function onChangeStartTimes(newVal,oldVal){
	...
	var lessonTime = dateTimeDifference($(startHourMinute.target).timespinner('getValue'), $(endHourMinute.target).timespinner('getValue'), "HH");
	$(lessonTimes.target).textbox('setValue',lessonTime);
} */
function onChangeLessonName(newVal,oldVal){
	if(newVal==undefined || oldVal==undefined){
		return false;
	}
	...
	var timeStarts = $(timeStart.target).datebox('getValue');
	var lessonTime = $(lessonTimes.target).textbox('getValue');
	var flag = lecturerHoursArrs(index,lessonTime,timeStarts,newVal);
	if(!flag){
		alert('每天的标准学时不能超过8小时！');
		$(lessonTimes.target).numberbox('setValue','');
		return false;
...
function onChangeLessonTimes(newVal,oldVal){
	if(newVal==undefined || oldVal==undefined){
		return false;
	}
	...
	var flag =true;
	if(aa>8  &&  newVal<=8){
		flag = lecturerHoursArrs(index,newVal,timeStarts,lecturerNumbers);
	}
	if(aa<=8  &&  newVal<=aa){
		flag = lecturerHoursArrs(index,newVal,timeStarts,lecturerNumbers);
	}
...
function dateTimeDifference(oneTime, twoTime, stype) {
	...
	//根据讲师培训日期、身份证号、学时数组
	function lecturerHoursArrs(index,hours,timeStarts,lecturerNumbers){ 
