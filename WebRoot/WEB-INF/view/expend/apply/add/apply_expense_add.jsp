	...
	">
		<thead>
			<tr>
				...
				<th data-options="field:'applyAmount',width:200,align:'center',editor:{type:'numberbox',options:{editable:true,precision:2,onChange:outAmounts}}">申请金额</th>
			</tr>
		</thead>
	</table>
</div>
<script type="text/javascript">
	...
	function expenseDetailJson(){
		...
		// 国际旅费
		var travelMoney =expenseRows[0].applyAmount;
		$('#travelMoney').val(travelMoney);
		var travel = JSON.stringify(expenseRows[0]);
		$('#travelJson').val(travel);
		// 国外城市间交通费
		var trafficMoney =expenseRows[1].applyAmount;
		$('#trafficMoney').val(trafficMoney);
		var outsideTraffic = JSON.stringify(expenseRows[1]);
		$('#outsideTrafficJson').val(outsideTraffic);
		// 住宿费
		var hotelMoney =expenseRows[2].applyAmount;
		...
		$('#foodMoney').val(foodObj.fApplyAmount);
		...
		$('#mixMoney').val(feeObj.fApplyAmount);
		...
		$('#feteMoney').val(feteObj.fApplyAmount);
		...
		$('#totalOtherMoney').val(otherObj.fCost);
