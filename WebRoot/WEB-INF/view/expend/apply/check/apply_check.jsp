						...
						<tr class="trbody">
							...
							<select id="fcostType" class="easyui-combobox" readonly="readonly"
								name="fcostType" style="width: 230px; height: 30px;" data-options="required:true" editable="false">
									<option value="1" <c:if test="${bean.fcostType=='1'}"> selected="selected" </c:if>>人员经费</option>
									<option value="2" <c:if test="${bean.fcostType=='2'}"> selected="selected" </c:if>>部门日常办公经费</option>
									<option value="3" <c:if test="${bean.fcostType=='3'}"> selected="selected" </c:if>>因公出国经费</option>
									<option value="4" <c:if test="${bean.fcostType=='4'}"> selected="selected" </c:if>>科研经费/教研经费</option>
									<option value="5" <c:if test="${bean.fcostType=='5'}"> selected="selected" </c:if>>其他公用经费</option>
							</select>
							</td>
						</tr>
					</table>
