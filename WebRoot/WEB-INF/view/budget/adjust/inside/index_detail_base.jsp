...
<div class="window-tab" style="margin-left: 0px;padding-top: 10px">
	...
	<div id="rpl" style="height:30px;padding-top : 8px">
		...
		<a href="${base}/project/outcomeDownload" id="index-imput">
			...
			<input type="hidden" id="outcomeTotal" name="outcomeTotal" value=""> 
		</a>
		<a style="text-align: right;">
			合计金额：<span style="color: red" id="outcomeTotalshow"><fmt:formatNumber groupingUsed="true" value="" minFractionDigits="2" maxFractionDigits="2"/></span>元
		</a>		
	</div>
	</c:if>
	<div class="win-left-bottom-div" style="height: 40px;">
		<a href="javascript:void(0)" onclick="saveIndexDetail()">
	...
	<form id="index_detail_base" method="post">
...
function projectOutcomeImput(){
	var win = creatSecondWin('支出明细导入', 500, 200, 'icon-search', '/project/outcomeImput');
	win.window('open');
}
...
function saveIndexDetail(){
	if(!getOutcomeJson()){
		alert("请填写全部信息！");
		return;
	}
	
	$('#index_detail_base').form('submit', {
			onSubmit: function(){
				
			}, 
			url:base+'/insideAdjust/saveDetail?id=${bean.FProId}', 
			success:function(data){
				
				pids = data;
				//重新加载表格内容
				$('#zbdr').datagrid('reload',{pids:pids});
