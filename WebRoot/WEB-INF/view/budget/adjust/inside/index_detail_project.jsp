			...
			<tr>
				...
				<th data-options="field:'subName',width:160,align:'center'
								,editor:{type:'textbox',options:{editable:false,icons:[{iconCls:'icon-add',handler: function(e){
									     ...
									     var index = $('#pro_outcomes_table_id').datagrid('getRowIndex',row);
...
//表格添加删除，保存方法
	...
	function appendOutcomeTable() {
		if (endEditing1()) {
			$('#pro_outcomes_table_id').datagrid('appendRow', {});
			editIndex1 = $('#pro_outcomes_table_id').datagrid('getRows').length - 1;
			$('#pro_outcomes_table_id').datagrid('selectRow', editIndex1).datagrid('beginEdit',editIndex1);
		}
	}
	function removeitOutcomeTable() {
		...
		$('#pro_outcomes_table_id').datagrid('cancelEdit', editIndex1).datagrid('deleteRow',
				editIndex1);
		editIndex1 = undefined;
	...
	function setFunSubCode(rec){
		...
		var ed = $('#pro_outcomes_table_id').datagrid('getEditor',{
			index:rindex,
			field : 'funSubCode'  
		});
	...
	function setSubCode(rec){
		var row = $('#pro_outcomes_table_id').datagrid('getSelected');
		var rindex = $('#pro_outcomes_table_id').datagrid('getRowIndex', row); 
		var administrationLevelCode = $('#pro_outcomes_table_id').datagrid('getEditor',{
			index:rindex,
...
function projectOutcomeImput(){
	...
	var win = creatSecondWin('支出明细导入', 500, 200, 'icon-search', '/project/outcomeImput?fProOrBasic='+fProOrBasic);
	win.window('open');
}
...
function chooseOutSubName(rIndex){
	...
	var code = '';
	var fProOrBasic = '${bean.FProOrBasic}';
	var planStartYear = '${bean.planStartYear}';
	if(planStartYear==''||planStartYear==null||planStartYear==undefined){
		alert('请先填写计划开始执行年份');
		return false;
	}
	var win=creatSecondWin('选择-支出预算事项',600,550,'icon-search','/insideAdjust/toChooseKm?rIndex='+rIndex+'&fProOrBasic='+fProOrBasic+'&ejProCode='+ejProCode+'&code='+code+'&ejplanStartYear='+planStartYear);
	win.window('open');
}
function chooseOutFun(rIndex){
	var win=creatSecondWin('选择-功能分类科目',600,550,'icon-search','/project/toChooseFun?rIndex='+rIndex);
	win.window('open');
}
function chooseOutCS(rIndex){
	var win=creatSecondWin('选择-功能分类科目',600,550,'icon-search','/project/toChooseCapitalSource?rIndex='+rIndex);
	win.window('open');
}
...
function onChangeSourceCode(newVal,oldVal){
	...
	if(flag){//如果资金来源表没有这个类型，就添加一行
			var row = $('#pro_outcomes_table_id').datagrid('getSelected');
			var rindex = $('#pro_outcomes_table_id').datagrid('getRowIndex', row); 
			var outAmount = $('#pro_outcomes_table_id').datagrid('getEditor',{
				index:rindex,
				field : 'outAmount'  
			});
			var capitalSourceName = $('#pro_outcomes_table_id').datagrid('getEditor',{
				index:rindex,
				field : 'capitalSourceName'
			});
			var capitalSourceCode = $('#pro_outcomes_table_id').datagrid('getEditor',{
				index:rindex,
				field : 'capitalSourceCode'  
			});
...
function capitalSourceCodeAndAmountArr(index,code){
	var arr = new Array();
	var outAmounts = $('#pro_outcomes_table_id').datagrid('getEditor',{
		index:index,
		field : 'outAmount'  
	});
...
function setOutcomesMoney(newValue,oldValue) {
	...
	var fsumMoney = 0;
	//获取编辑行
	var index = $('#pro_outcomes_table_id').datagrid('getRowIndex',$('#pro_outcomes_table_id').datagrid('getSelected'));
	//获取总行数
	var rows = $('#pro_outcomes_table_id').datagrid('getRows');
	for(var i=0;i<rows.length;i++){
