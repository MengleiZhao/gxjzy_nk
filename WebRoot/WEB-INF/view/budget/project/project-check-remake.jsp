...
<div style="height: 427px;width: 550px;">
	<div style="width: 496px;height: 391px">
		<div style="margin-left: 20px;overflow:auto">
			<table cellpadding="0" cellspacing="0" style="width: 476px">
				<tr style="height: 40px;">
					<td>项目名称</td>
					<td id="checkProject3"></td>
				</tr>
				<tr style="height: 40px;">
					<td>项目编号</td>
					<td id="checkProject4"></td>
				</tr>
				<tr style="height: 40px;">
					<td>审批结果</td>
					<td>
				...
				<tr style="height: 40px;">
					<td>是否上传附件</td>
					<td>
						<input id="filecheck" onclick="changefile()" type="checkbox" value="0" >
					</td>
				</tr>
				<tr hidden="hidden" id ="checkid">
					<td>
						&nbsp;&nbsp;
						<input type="file" multiple="multiple" id="fspjl" onchange="check_File(this,'spjl','spjl01')" hidden="hidden">
					...
					<td colspan="4" id="tdspjl">
						...
						<c:forEach items="${CheckAttaList}" var="att">
							<c:if test="${att.serviceType=='spjl' }">
								...
								&nbsp;&nbsp;&nbsp;&nbsp;
									<a id="${att.id}" class="fileUrl1" href="#" style="color:red" onclick="deleteAttac(this)">删除</a>
								</div>
							</c:if>
						</c:forEach>
	...
	<div style="width: 496px;height: 66px;line-height: 66px;text-align: center;">
		<a href="javascript:void(0)" onclick="remakeSave('${result}')">
...
$(document).ready(function() {
	$('#checkProject3').html($('#project_add_FProName').textbox().textbox('getValue'));//项目名称
	$('#checkProject4').html($('#project_add_FProCode').textbox().textbox('getValue'));//项目编号
});

function remakeSave(iftg) {
	//绩效指标
	var totalityPerformanceJson = $('#totalityPerformanceJson').val();
	
	//验证：通过时无提示，不通过且原因没填时提示
	var spjlFiles=$('#spjlFiles').val();
	if ('${result}'=='0' && $('#projectCheckRemake').val()=='') {
	...
	if ('${result}'=='0' && $('#projectCheckRemake').val()=='') {
		alert('审批不通过时，请填写审批意见！');
		return;
	}else if(filecheck==1&&spjlFiles==''){
		alert('请上传文件');
	}else if (confirm('是否确定保存？')) {
		//type为yssp一上审批essp二上审批
		var remake = $('#projectCheckRemake').val();
		$('#fcheckRemake').val(remake);
		$('#fcheckResult').val(iftg);
		$('#spjlFile').val(spjlFiles);
		$('#project_verdict_form').form('submit', {
			...
			success:function(data){
				...
				if(data.success){
					...
					closeFirstWindow();
				  	...
				  	/* closeWindow();
					$('#project_verdict_form').form('clear'); */
				}
			} 
		});	
	}
	    	 ...
	    	 if(jsonResponse.success==true){
	    		 		...
	    		 		var name = inf[1];//附件名称、
	    		 		 ...
	    		 		 $('#tdspjl').append(
	 			    			"<div style='margin-top: 10px;'>"+
	 			    				...
	 			    				"&nbsp;&nbsp;&nbsp;&nbsp;<a id='"+id+"' class='fileUrl1' href='#' style='color:red' onclick='deleteAttac(this)'>删除</a><div>"
	 			    		);
	    		...
	    		$('#tdspjl').append(
	    				"<div style='margin-top: 10px;'>"+
	    					...
	    					"&nbsp;&nbsp;&nbsp;&nbsp;<a id='fail' class='fileUrl1' href='#' style='color:red' onclick='deleteAttac(this)'>删除</a><div>"
	    			);
	    	}
      } else {
          alert("上传失败");
