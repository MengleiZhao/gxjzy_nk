...
<%@ include file="/includes/taglibs.jsp"%>
	...
	<div class="easyui-accordion" style="margin-left: 20px;margin-right: 20px;width: 990px;height:auto">
		...
		<div style="overflow:auto;margin-top: 0px;">
			<table id="performance" class="easyui-datagrid" style="width:980px;height:auto"
						data-options="
						singleSelect: true,
						toolbar: '#performance_toolbar_Id',
						rownumbers : true,
						...
						<th data-options="field:'tTwoName',align:'center',editor:{
										...
										options:{
											...
											onShowPanel:function(){
												...
												var index=$('#performance').datagrid('getRowIndex',$('#performance').datagrid('getSelected'));
												var tr = $('#performance').datagrid('getEditors', index);
												var tOneCode = $('#performance').datagrid('getEditor',{
													index:index,
													field:'tOneCode'
												});
												var tTwoName = $('#performance').datagrid('getEditor',{
													index:index,
													field:'tTwoName'
												});
		...
		<div id="performance_toolbar_Id" style="height:30px;width: 865px;margin-top: 5px;">
			...
			<a style="float: right;">&nbsp;&nbsp;</a>
			<a style="float: right;" href="#" onclick="exportXmPerformance()">
...
//存入json
function getPerformanceJsonExport(){
	performanceAccept();
	var rows = $('#performance').datagrid('getRows');
	var performance = "";
	for (var j = 0; j < rows.length; j++) {
	...
	$('#form_pro_export_json').val(performance);
}


function onLoadSuccessPerFormance(){
	$('#performance').datagrid('appendRow', {
		tOneCode:1,
		tOneName:'产出指标',
		tTwoCode:1,
...
function exportXmPerformance(){
	if(confirm('是否导出？')){
		getPerformanceJsonExport();
		$('#form_pro_export_performance').attr('action','${base}/project/exportPerformance');
		$('#form_pro_export_performance').submit();
	}
}
</script>
