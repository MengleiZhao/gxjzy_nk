...
<div class="list-div">
	...
	<div class="list-table">
		<table id="pro_dg_${listid}" class="easyui-datagrid"
			data-options="collapsible:true,url:'${base}/project/projectPageData?proLibType=${proLibType}&sbkLx=${sbkLx}'<c:if test="${listid==1}">+'&FProOrBasic=2'</c:if><c:if test="${listid==1}">+'&FFlowStauts=11'</c:if>,
...
function format_oper(value, row, index){
	...
	var sbkLx = '${sbkLx }';//申报库类型 项目申报xmsb审批zxmsp台账xmtz(页面跳转参数)
	var btn = "";
	...
	}else if(proLibType=='1' && sbkLx=='zxmsp'){//申报库-项目审批库 显示审批
		//审批
