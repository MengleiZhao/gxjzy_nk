...
<div class="list-div">
	...
	<div class="list-top">
		<table class="top-table" cellpadding="0" cellspacing="0">
			<tr>
				<td style="width: 83%;padding-left: 0px" class="queryth">
					<div class="top-table-search-td">	
						...
						<input id="pro_list_query_FProName"  style="width: 150px;height:25px;" class="easyui-textbox"/>
		     			收报状态&nbsp;
						<select id="essb_FExportStauts" class="easyui-combobox" data-options="required:true,editable:false" name="FExportStauts" style="height:25px; width: 150px;">
		     				<option value="">--请选择--</option>
		     				<option value="0">未收报</option>
		     				<option value="1">已收报</option>
		     			</select>
						</br>
					</div>
				...
				<td align="right" style="padding-right: 0px">
				    ...
				    <a href="#" onclick="receiveReport(1)">
						<img style="vertical-align:bottom" src="${base}/resource-modality/${themenurl}/button/bbsb1.png" onMouseOver="mouseOver(this)" onMouseOut="mouseOut(this)">
					</a>
				    <a href="#" onclick="receiveReport(2)">
						<img style="vertical-align:bottom" src="${base}/resource-modality/${themenurl}/button/cxsb1.png" onMouseOver="mouseOver(this)" onMouseOut="mouseOut(this)">
					</a>
...
function format_protzlist(value, row, index){
	...
	var fflowStauts = row.fflowStauts;
	var btn ="";
	if(rows==0 || rows==2){
		btn =  "<table><tr style='width: 105px; height:20px'>"
				...
				+ "</a></td>";
		if(fflowStauts==19){//一上审批通过,预算管理岗可以撤回
			btn=btn+"<td style='width:25px'><a href='#' onclick='reCall(\"pro_tz_dg\","+row.fproId+",\"/project/reCall\")'>"
...
function queryProLedger(){
	...
	$("#pro_tz_dg").datagrid('load',{
		...
		FExportStauts:$("#essb_FExportStauts").combobox('getValue').trim(),
...
function clearProTable(){
	...
	$("#essb_FExportStauts").combobox('setValue','');
...
function receiveReport(stauts){
	...
	for (var i = 0; i < checkedItems.length; i++) {
		if(checkedItems[i].fexportStauts == 1 && stauts==1){
