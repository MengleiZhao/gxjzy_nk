		...
		<table cellpadding="0" cellspacing="0">
			...
			<tr class="trbody">
   			...
   			<tr class="trbody">
   				...
   				<td class="td2">
		     		<input class="easyui-combobox" style="height:30px; width: 300px" required="true" <c:if test="${bean.FProAppliDepartId!='13'}"> readonly="readonly"</c:if> data-options="editable:false,url:'${base}/Formulation/lookupsJson?parentCode=PRO-TYPE<c:if test="${bean.FProAppliDepartId=='13'}">&selected=${bean.FProClass}</c:if><c:if test="${bean.FProAppliDepartId!='13'}">&selected=PRO-TYPE-01</c:if>',method:'POST',valueField:'code',textField:'text'"/>
   				</td>
   			</tr>
			<tr class="trbody">
			...
			<tr class="trbody" id="base_secondLevel" >
				...
				<td class="td2">
		     		<input type="radio" value="1" name="fprocurementStatus" disabled="disabled" onclick="$('#zhengfuId').css('display','');$.parser.parse('#mingxiId');$('#pro_purchase_tab_id').datagrid('reload');" id="box1" <c:if test="${bean.fprocurementStatus=='1'}">checked="checked" </c:if> style="vertical-align: middle;"/>&nbsp;&nbsp;是
					&nbsp;&nbsp;
					<input type="radio" value="0" name="fprocurementStatus" disabled="disabled" onclick="$('#zhengfuId').css('display','none');" id="box2" <c:if test="${bean.fprocurementStatus=='0'||bean.fprocurementStatus==''}">checked="checked" </c:if> style="vertical-align: middle;"/>&nbsp;&nbsp;否
				</td> 
		     	<td class="td3-ys"></td>
				<td class="td1-ys"></td>
		...
		<table cellpadding="0" cellspacing="0" >
			<tr class="trbody">
		    	<td class="td1-ys" style="padding-top: 0px" colspan="6">
		    		<%@ include file="../project/detail/project-detail-fundssource-two.jsp" %>
				</td>
		    </tr>
		</table>
	</div>
