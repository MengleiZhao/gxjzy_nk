...
<div class="window-tab" style="margin-left: 0px;padding-top: 10px">
	<table id="pro_outcomes_table_essb_id" class="easyui-datagrid" 
	...
	onClickRow: onClickRowProOutcomesEssb,
	</c:if>
	striped : true,
	nowrap : false,
	rownumbers:true,
	scrollbarSize:0,
	onLoadSuccess:onLoadSuccessOutcomes
	">
		<thead>
			<tr>
				...
				<th data-options="field:'subName',width:160,align:'center'
								,editor:{type:'textbox',options:{editable:false,icons:[{iconCls:'icon-add',handler: function(e){
									     var row = $('#pro_outcomes_table_essb_id').datagrid('getSelected');
									     var index = $('#pro_outcomes_table_essb_id').datagrid('getRowIndex',row);
									     chooseOutSubNameEssb(index)
									     }}]}}">经济分类科目</th>
				<th data-options="field:'subCode',hidden:true,width:150,align:'center',editor:'textbox'">经济分类科目编号</th>
				<th data-options="field:'capitalSourceName',width:160,align:'center'">资金来源</th>
				<th data-options="field:'capitalSourceCode',hidden:true,width:150,align:'center'">资金来源编号</th>
				<th data-options="field:'outAmount',width:130,align:'center',editor:{type:'numberbox',options:{onChange:setOutcomesMoneyEssb,precision:2}}">支出金额(元)</th>
	...
	<div id="rpl" style="height:30px;padding-top : 8px">
		...
		<a href="javascript:void(0)" onclick="removeitOutcomeTableEssb()" id="rrmoveid" style="float: right;"><img src="${base}/resource-modality/${themenurl}/button/scyh1.png" onMouseOver="mouseOver(this)" onMouseOut="mouseOut(this)"></a>
		<a style="float: right;">&nbsp;&nbsp;</a>
		<a href="javascript:void(0)" onclick="appendOutcomeTableEssb()" id="raddid" style="float: right;"><img src="${base}/resource-modality/${themenurl}/button/tjyh1.png" onMouseOver="mouseOver(this)" onMouseOut="mouseOut(this)"></a>

		<a href="#" id="index-imput" onclick="projectOutcomeImputEssb()">
...
//表格添加删除，保存方法
	var editIndexEssb1 = undefined;
	function endEditing1Essb() {
		if (editIndexEssb1 == undefined) {
			return true
		}
		if ($('#pro_outcomes_table_essb_id').datagrid('validateRow', editIndexEssb1)) {
			var tr = $('#pro_outcomes_table_essb_id').datagrid('getEditors', editIndexEssb1);
			...
			$('#pro_outcomes_table_essb_id').datagrid('endEdit', editIndexEssb1);
			editIndexEssb1 = undefined;
			return true;
		} else {
			return false;
		}
	}
	function onClickRowProOutcomesEssb(index) {
		if (editIndexEssb1 != index) {
			if (endEditing1Essb()) {
				$('#pro_outcomes_table_essb_id').datagrid('selectRow', index).datagrid('beginEdit',
						index);
				editIndexEssb1 = index;
			} else {
				$('#pro_outcomes_table_essb_id').datagrid('selectRow', editIndexEssb1);
			}
		}
	}
	function appendOutcomeTableEssb() {
		if (endEditing1Essb()) {
			$('#pro_outcomes_table_essb_id').datagrid('appendRow', {});
			editIndexEssb1 = $('#pro_outcomes_table_essb_id').datagrid('getRows').length - 1;
			$('#pro_outcomes_table_essb_id').datagrid('selectRow', editIndexEssb1).datagrid('beginEdit',editIndexEssb1);
		}
	}
	function removeitOutcomeTableEssb() {
		if (editIndexEssb1 == undefined) {
			return
		}
		$('#pro_outcomes_table_essb_id').datagrid('cancelEdit', editIndexEssb1).datagrid('deleteRow',
				editIndexEssb1);
		editIndexEssb1 = undefined;
		allRowsProMingXiAmountEssb();
	}
	function accept1Essb() {
		if (endEditing1Essb()) {
			$('#pro_outcomes_table_essb_id').datagrid('acceptChanges');
		}
	}
	
	function setFunSubCodeEssb(rec){
		var row = $('#pro_outcomes_table_essb_id').datagrid('getSelected');
		var rindex = $('#pro_outcomes_table_essb_id').datagrid('getRowIndex', row); 
		var ed = $('#pro_outcomes_table_essb_id').datagrid('getEditor',{
			index:rindex,
			field : 'funSubCode'  
		});
		$(ed.target).textbox('setValue', rec.code);
	}
	function setSubCodeEssb(rec){
		var row = $('#pro_outcomes_table_essb_id').datagrid('getSelected');
		var rindex = $('#pro_outcomes_table_essb_id').datagrid('getRowIndex', row); 
		var administrationLevelCode = $('#pro_outcomes_table_essb_id').datagrid('getEditor',{
			index:rindex,
			field : 'administrationLevelCode'  
		});
			$(administrationLevelCode.target).textbox('setValue', rec.code);
	}
	function setCapitalSourceCodeEssb(rec){
		var row = $('#pro_outcomes_table_essb_id').datagrid('getSelected');
		var rindex = $('#pro_outcomes_table_essb_id').datagrid('getRowIndex', row); 
		var code = $('#pro_outcomes_table_essb_id').datagrid('getEditor',{
			index:rindex,
...
//弹出支出明细导入页面
function projectOutcomeImputEssb(){
	
	var fProOrBasic = '${bean.FProOrBasic}';
	var win = creatFirstWin('支出明细导入', 500, 200, 'icon-search', '/project/outcomeImput?fProOrBasic=2');
	win.window('open');
}

function chooseOutSubNameEssb(rIndex){
	var ejProCode="";
	var code = '';
	var fProOrBasic = '${bean.FProOrBasic}';
	...
	var win=creatFirstWin('选择-支出预算事项',600,550,'icon-search','/project/toChooseKmEssb?rIndex='+rIndex+'&fProOrBasic='+fProOrBasic+'&ejProCode='+ejProCode+'&code='+code+'&ejplanStartYear='+planStartYear);
	win.window('open');
}
...
function onChangeSourceCodeEssb(newVal,oldVal){
	if(newVal==undefined || oldVal==undefined){
		return false;
	}
	
	var flag = comparativeAnalysisEssb(newVal);
	if(flag){//如果资金来源表没有这个类型，就添加一行
			var row = $('#pro_outcomes_table_essb_id').datagrid('getSelected');
			var rindex = $('#pro_outcomes_table_essb_id').datagrid('getRowIndex', row); 
			var outAmount = $('#pro_outcomes_table_essb_id').datagrid('getEditor',{
				index:rindex,
				field : 'outAmount'  
			});
			var capitalSourceName = $('#pro_outcomes_table_essb_id').datagrid('getEditor',{
				index:rindex,
				field : 'capitalSourceName'
			});
			var capitalSourceCode = $('#pro_outcomes_table_essb_id').datagrid('getEditor',{
				index:rindex,
				field : 'capitalSourceCode'  
			});
			...
			$('#fundssourceEssb').datagrid('appendRow',{
...
function comparativeAnalysisEssb(data){
	var flag = true;
	var rows = $('#fundssourceEssb').datagrid('getRows');
	for (var i = 0; i < rows.length; i++) {
...
function capitalSourceCodeAndAmountArr(index,code){
	var arr = new Array();
	var outAmounts = $('#pro_outcomes_table_essb_id').datagrid('getEditor',{
		index:index,
		field : 'outAmount'  
	});
	var capitalSourceCodes = $('#pro_outcomes_table_essb_id').datagrid('getEditor',{
		index:index,
		field : 'capitalSourceCode'  
	});
	var idAndName = {};
...
//计算总额
function setOutcomesMoneyEssb(newValue,oldValue) {
	...
	var fsumMoney = 0;
	//获取编辑行
	var index = $('#pro_outcomes_table_essb_id').datagrid('getRowIndex',$('#pro_outcomes_table_essb_id').datagrid('getSelected'));
	//获取总行数
	var rows = $('#pro_outcomes_table_essb_id').datagrid('getRows');
	for(var i=0;i<rows.length;i++){
	...
	$('#pro_add_FProBudgetAmount').val(totalFsumMoney.toFixed(2));
	$('#outcomeTotalshow').html(totalFsumMoney.toFixed(2));
}
//未编辑或者已经编辑完毕的行
function addNumOutcomesEssb(rows,index){
	//获取资金来源中金额列的值
...
function allRowsProMingXiAmountEssb(){
	//总金额
	var totalFsumMoney = 0;
	var rows = $('#pro_outcomes_table_essb_id').datagrid('getRows');
	for(var i=0;i<rows.length;i++){
...
function onLoadSuccessOutcomes(){
	//总金额
	var totalFsumMoney = 0;
	var rows = $('#pro_outcomes_table_essb_id').datagrid('getRows');
	for(var i=0;i<rows.length;i++){
