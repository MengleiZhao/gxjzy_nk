...
<div class="window-tab" style="margin-left: 20px;padding-top: 20px">
	<table id="pro_outcomes_detail_table" class="easyui-datagrid" style="width:870px;height:auto"
	data-options="
	singleSelect: true,
	toolbar: '#rpl',
	url: '${base}/project/findByfProIdGetDetail?id=${bean.FProId}',
	method: 'post',
	<c:if test="${operation=='verdict'}">
	</c:if>
	<c:if test="${!empty checkPeople}">
	onClickRow: onClickRowProOutcomesTwo,
	onLoadSuccess:onLoadSuccesscapitalSourceCodeAndAmountArrTwo,
	</c:if>
	striped : true,
	nowrap : false,
	rownumbers:true,
	...
	<input type="hidden" id="outcomeJsons" name="outcomeJsons" />
</div>
...
//表格添加删除，保存方法
	...
	function endEditing1() {
		...
		if ($('#pro_outcomes_detail_table').datagrid('validateRow', editIndex1)) {
			var tr = $('#pro_outcomes_detail_table').datagrid('getEditors', editIndex1);
			var text0=tr[0].target.combotree('getText');
			if(text0!='--请选择--'){
				tr[0].target.combotree('setValue',text0);
			}
			$('#pro_outcomes_detail_table').datagrid('endEdit', editIndex1);
	...
	function onClickRowProOutcomesTwo(index) {
		if (editIndex1 != index) {
			if (endEditing1()) {
				$('#pro_outcomes_detail_table').datagrid('selectRow', index).datagrid('beginEdit',
						index);
				editIndex1 = index;
			} else {
				$('#pro_outcomes_detail_table').datagrid('selectRow', editIndex1);
			}
		}
	}
	function accept1() {
		if (endEditing1()) {
			$('#pro_outcomes_detail_table').datagrid('acceptChanges');
		}
	}
	
	function setCapitalSourceCode(rec){
		
		var row = $('#pro_outcomes_detail_table').datagrid('getSelected');
		var rindex = $('#pro_outcomes_detail_table').datagrid('getRowIndex', row); 
		var code = $('#pro_outcomes_detail_table').datagrid('getEditor',{
			index:rindex,
...
function capitalSourceCodeAndAmountArr(index,code){
	...
	var rows = $('#pro_outcomes_detail_table').datagrid('getRows');
	var arr = new Array();
	var capitalSourceCodes = $('#pro_outcomes_detail_table').datagrid('getEditor',{
		index:index,
		field : 'capitalSourceCode'  
	});
	var capitalSourceNames = $('#pro_outcomes_detail_table').datagrid('getEditor',{
		index:index,
...
//计算资金来源和合计金额
function onLoadSuccesscapitalSourceCodeAndAmountArrTwo(){
	var rows = $('#pro_outcomes_detail_table').datagrid('getRows');
	var arr = new Array();
	for (var i = 0; i < rows.length; i++){
			var capitalSourceCode = rows[i].capitalSourceCode;
    ...
    $('#fundJson').val(idAndNameJsons);
    $('#fundssourceDetail').datagrid('loadData',globalVariable);
	return idAndNames;
}
</script>