...
<div class="list-div">
	<div style="height: 10px;background-color:#f0f5f7 "></div>
		...
		<div class="list-table">
			...
			data-options="collapsible:true,url:'${base}/Storage/approvalJson',
...
function mouseOut(img) {
	...
	function storage_fixed_add() {
		...
		var win = creatWin('资产增加', 970,580, 'icon-search', '/Storage/fixedadd');
		if (null != node) {
			win = creatWin('资产增加', 970,580, 'icon-search',
					'/Storage/fixedadd');
		}
		win.window('open');
	}
	function storage_fixed_detail(id) {
			var win = creatWin('查看', 1040 ,580, 'icon-search',"/Storage/detail/" + id);
			win.window('open');
	}
	/* function editCF() {
	...
	function storage_fixed_sp(id) {
		//var row = $('#storage_fixed_approval_dg').datagrid('getSelected');
		var selections = $('#storage_fixed_approval_dg').datagrid('getSelections');
		var win = creatWin('审批', 970, 580, 'icon-search',
				"/Storage/approvel/" + id);
		win.window('open');
	}
