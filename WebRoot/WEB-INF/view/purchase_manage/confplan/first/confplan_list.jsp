...
function addConfplan() {
	var win = parent.creatWin('新增', 970, 580, 'icon-search', '/cgconfplan/add?freportStage='+reportStage);
	win.window('open');
}
...
function confplan_update(id) {
	var win = parent.creatWin('修改', 970, 580, 'icon-search', '/cgconfplan/edit?id='+id);
	win.window('open'); 
}
...
function confplan_delete(id) {
	if (confirm('确认删除吗？')) {
		$.ajax({
			type : 'POST',
			url : '${base}/cgconfplan/delete?id='+id,
			dataType : 'json',
