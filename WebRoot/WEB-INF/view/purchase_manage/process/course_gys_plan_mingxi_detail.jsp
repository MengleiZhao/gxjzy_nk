...
<table id="course_gys_detail_dg" class="easyui-datagrid" style="margin-top: 10px;width:695px;height:auto;"
data-options="
singleSelect: true,
toolbar: '#course_gys_detail_tb',
rownumbers : true,
striped:true,
...
<thead>
	<tr>
		...
		<th data-options="field:'fnum',align:'center',width:70,editor:{type:'numberbox',options:{readonly:true}}">数量</th>
		<th data-options="field:'funitPrice',align:'center',width:110,editor:{type:'numberbox',options:{precision:2,readonly:true}}">单价(元)</th>
		<th data-options="field:'fsumMoney',align:'center',width:110,editor:{type:'numberbox',options:{readonly:true,precision:2,onChange:setFunitPrice}}">中标金额(元)</th>
		<th data-options="field:'fcommProp',align:'center',width:190">备注</th>
	</tr>
</thead>
</table>
<div id="course_gys_detail_tb" style="height:30px;margin-bottom: 5px;">
...
//加载完以后自动计算金额
$('#course_gys_detail_dg').datagrid({onLoadSuccess : function(data){
	setFunitPrice();
}});
...
function endEditing() {
	...
	if ($('#course_gys_detail_dg').datagrid('validateRow', editIndex)) {
		var ed = $('#course_gys_detail_dg').datagrid('getEditor', {
			index : editIndex,
			field : 'costDetail'
		});
		$('#course_gys_detail_dg').datagrid('endEdit', editIndex);
		editIndex = undefined;
		return true;
	} else {
		return false;
...
function onClickRow(index) {
	if (editIndex != index) {
		if (endEditing()) {
			$('#course_gys_detail_dg').datagrid('selectRow', index).datagrid('beginEdit', index);
			editIndex = index;
		} else {
			$('#course_gys_detail_dg').datagrid('selectRow', editIndex);
		}
	}
}
function course_gys_append() {//未配置采购类型不可添加采购清单
	if (endEditing()) {
		$('#course_gys_detail_dg').datagrid('appendRow', {});
		editIndex = $('#course_gys_detail_dg').datagrid('getRows').length - 1;
		$('#course_gys_detail_dg').datagrid('selectRow', editIndex).datagrid('beginEdit', editIndex);
	} 
...
function course_gys_removeit() {
	...
	$('#course_gys_detail_dg').datagrid('cancelEdit', editIndex).datagrid('deleteRow',
			editIndex);
	editIndex = undefined;
	//修改申请总额
	setFunitPrice(0,0);
...
function accept() {
	if (endEditing()) {
		$('#course_gys_detail_dg').datagrid('acceptChanges');
	}
}
//计算总额
function setFunitPrice(newValue,oldValue) {
	...
	var fsumMoney = 0;
	var index=$('#course_gys_detail_dg').datagrid('getRowIndex',$('#course_gys_detail_dg').datagrid('getSelected'));
	var rows = $('#course_gys_detail_dg').datagrid('getRows');
	...
	var ed = $('#course_gys_detail_dg').datagrid('getEditor', {index:index,field:'funitPrice'});
	if(index!=-1){
...
function setEditing(rows,index){
    var editors = $('#course_gys_detail_dg').datagrid('getEditors', index);  
    var fnum = editors[1]; 
