...
<div class="list-div">
	...
	<div class="list-table">
		<table id="ContractRevenueTab" class="easyui-datagrid"
		data-options="collapsible:true,url:'${base}/proceedsPlan/JsonPaginationCheck',
		...
		selectOnCheck: true,checkOnSelect:true,remoteSort:true,nowrap:false,striped: true,fitColumns:true" >
			<thead>
					<tr>
						...
						<th data-options="field:'fcTitle',align:'center',resizable:false,sortable:true" width="16%">合同名称</th>
						<th data-options="field:'bcSkAmount',align:'right',resizable:false,sortable:true,formatter:function(value,row,index){return fomatMoney(value,2);}" width="12%">本次收款金额(元)</th>
						...
						<th data-options="field:'fProceeds',align:'center'" width="10%">收款性质</th>
...
function CZ(val, row) {
	return 	'<table><tr style="width: 75px;height:20px"><td style="width: 25px">'+
			'<a href="#" onclick="checkPro(' + row.fcId + ',' + row.fPlanId + ')" class="easyui-linkbutton">'+
			'<img onmouseover="listMouseOver(this)" onmouseout="listMouseOut(this)" src="'+base+'/resource-modality/${themenurl}/list/check1.png">'+
			'</a></td></tr></table>';
	/* if (row.col8=='1') {
		return '<table><tr style="width: 75px;height:20px"><td style="width: 25px">'+
			   '<a href="#" onclick="detailAccounts(' + row.id + ',\'' + row.col1 + '\',\'' + row.col9 + '\')" class="easyui-linkbutton">'+
		...
		return 	'<table><tr style="width: 75px;height:20px"><td style="width: 25px">'+
				...
				'<a href="#" onclick="editAccounts(' + row.id + ',\'' + row.col1 + '\',\'' + row.col9 + '\')" class="easyui-linkbutton">'+
				'<img onmouseover="listMouseOver(this)" onmouseout="listMouseOut(this)" src="'+base+'/resource-modality/${themenurl}/list/qr1.png">'+
				'</a>' + '</td></tr></table>';
} */

}
function checkPro(id,fPlanId) {
	var win = creatWin('审批', 1115, 580, 'icon-search', '/proceedsPlan/checkPro/?id='+id+'&fPlanId='+fPlanId);
	win.window('open');
}
//修改
function editAccounts(id,cId,type) {
	var win = null;
	win = creatWin('来款确认-修改', 795, 600, 'icon-search', "/proceedsPlan/editCheck/"+ id+'?cId='+cId+'&type='+type);
	 win.window('open');	
}
//查看
