			...
			selectOnCheck: true,checkOnSelect:true,remoteSort:true,nowrap:false,striped: true,fitColumns: true" >
				<thead>
					<tr>
						...
						<th data-options="field:'fProceedsCondition',align:'center',resizable:false" width="20%">收款条件</th>
						<th data-options="field:'fProceedsAmount',align:'right',resizable:false" width="15%">预计收款金额(元)</th>
						<th data-options="field:'fProceedsTime',align:'center',formatter: ChangeDateFormat" width="20%" >预计收款日期</th>
						<th data-options="field:'fStauts',align:'center',resizable:false,formatter:RPsatuts" width="15%">收款状态</th>
...
/* function RPsatuts(val, row) {
	...
	function CZ(val, row) {
		if(row.payStauts==''||row.payStauts == null){
			return '<a href="#" onclick="pay(' + ${id}+ ',' + row.fPId + ')" class="easyui-linkbutton"><img onmouseover="listMouseOver(this)" onmouseout="listMouseOut(this)"  src="'+base+'/resource-modality/${themenurl}/list/pay1.png">' + '</a>';
		...
		$.ajax({ 
			...
			success: function(data){ 
				if(data=='2'){
					...
					var win = creatFirstWin('合同付款申请', 1115, 580, 'icon-search',"/proceedsPlan/add?id=" + fPlanId+"&fPlanId="+Id);
					win.window('open');
				}
			} 
		}); 	
	...
	function edit(id,fPlanId) {
		var win = creatFirstWin('合同付款申请-修改', 1115, 580, 'icon-search',"/proceedsPlan/edits?id=" + id+"&fPlanId="+fPlanId);
		win.window('open');
	} 
	function enforcing_plan_detail(id,fPlanId) {
		var win = creatFirstWin('合同付款明细', 1115, 580, 'icon-search',"/proceedsPlan/detailPro?id=" + id+"&fPlanId="+fPlanId);
		win.window('open');
	}
	/* //查看申请
