...
<body>
<div class="win-div">
<form id="economicAddEditForm" action="${base}/economic/save" method="post" data-options="novalidate:true" class="easyui-form" enctype="multipart/form-data">
					...
					<div title="经济科目" id="djdxqDiv" data-options="iconCls:'icon-xxlb',collapsed:false,collapsible:false" style="overflow:auto;margin-top:10px;">
						<table class="ourtable">
							...
							<tr class="trbody">
								...
								<td class="td2">
									<input id="economic_leve" <c:if test="${openType=='edit'}">readonly="readonly"</c:if> name="leve" style="width: 200px;" value="${bean.leve}" data-options="url:'${base}/economic/lookupsJson?parentCode=KMJB&selected=${bean.leve}',method:'POST',valueField:'code',textField:'text',editable:false,validType:'selectValid'" class="easyui-combobox">
								</td>
								<td class="td4">&nbsp;</td>
							...
							<tr class="trbody">
								...
								<td class="td2">
									<input class="easyui-combobox" id="economic_type" <c:if test="${openType=='edit'}">readonly="readonly"</c:if> name="type" value="${bean.type}" style="width: 200px;" data-options="url:'${base}/economic/lookupsJson?parentCode=KMLX&selected=${bean.type}',method:'POST',valueField:'code',textField:'text',editable:false,validType:'selectValid'" class="easyui-combobox">
								</td>
								<td class="td4">&nbsp;</td>
...
<script type="text/javascript">
		...
		/* 	//校验
			...
			if($title=="") 
			{
				...
				var flag = true;
			    if(!r.test($('#economic_code').val()) && flag == true){
			    	alert("科目编号请输入正整数！");
			    	flag = false;
			    } 	
			    if(!r.test($('#economic_id').val()) && flag == true){
			    	alert("上级科目编号请输入正整数！");
			    	flag = false;
			    } 	
			 */
