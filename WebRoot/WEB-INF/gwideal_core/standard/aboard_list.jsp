...
<div class="list-div">
	<div style="height: 10px;background-color:#f0f5f7 "></div>
		...
		<div class="list-table">
			...
			data-options="collapsible:true,url:'${base}/hotelStandard/pageList?outType=aboard',
			...
			selectOnCheck: true,checkOnSelect:true,remoteSort:true,nowrap:false,striped: true,fitColumns: true" >
				<thead>
					<tr>
						...
						<th data-options="field:'costTraffic',align:'right'" width="25%">国外交通补助[元]</th>
						<th data-options="field:'costPocket1',align:'right'" width="25%">零用费（10天内）[元]</th>
						<th data-options="field:'costPocket2',align:'right'" width="25%">零用费（第11天及以后）[元]</th>
						<th data-options="field:'operation',align:'center',resizable:false,sortable:true,formatter:aboardStdOpe" width="15%">操作</th>
					</tr>
				</thead>
			</table>
		</div>
...
function query_hstd() {
	var aboard_ = $('#htsd_area').textbox('getValue');
...
function add_hstd() {
	...
	var win = creatWin('新增', 705, 580, 'icon-search', '/hotelStandard/add?outType=aboard');
	win.window('open');
}

function detail_hstd(id) {
		var win = creatWin('查看', 705, 580, 'icon-search',"/hotelStandard/detail/" + id + "?outType=aboard");
		win.window('open');
}

function update_hstd(id) {
	//var row = $('#hotestd_dg').datagrid('getSelected');
	var win = creatWin('修改', 705, 580, 'icon-search',"/hotelStandard/edit/" + id + "?outType=aboard");
	win.window('open');
}
...
function delete_hstd(id) {
	...
	if (confirm("确认删除吗？")) {
		$.ajax({
			type : 'POST',
			url : '${base}/hotelStandard/delete/' + id +'?outType=aboard',
			dataType : 'json',
