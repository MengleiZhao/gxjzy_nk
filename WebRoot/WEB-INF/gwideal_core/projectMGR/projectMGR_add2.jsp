...
.trbody{height: 30px;}
	...
	function deleteAttac(a){
		...
		function PM2AddEditForm(){
			/* //附件的路径地址
			var s="";
			$(".fileUrl").each(function(){
				s=s+$(this).attr("id")+",";
			}); */
			$('#PM2AddEditForm').form('submit', {
				onSubmit: function(){ 
    ...
    <div class="easyui-layout" fit="true" style="color: #333333">
   		<div data-options="region:'west',split:true"style="width:600px;border-color: dce5e9" id="westDiv">
	    	<form id="PM2AddEditForm" action="${base}/ProMgrLevel/save2" method="post" data-options="novalidate:true" class="easyui-form" enctype="multipart/form-data">
	    		<input type="hidden" name="fLvId2" id="PM_fLvId2" value="${bean.fLvId2}"/>
	    		<input type="hidden" name="fEcName" id="PM_fEcName" value="${bean.fEcName}"/>
	    		<input type="hidden" name="a" id="PM_a" value="${bean.pml.fLvId1}"/>
				<table >
				<tr>
					<td >
						<div class="easyui-accordion" style="width:555px;margin-left: 20px;">
							<div title="二级详情" data-options="iconCls:'icon-xxlb',collapsed:false,collapsible:false" style="overflow:auto;margin-top:10px;">
								<table class="ourtable" cellpadding="0" cellspacing="0">
									<tr class="trbody">
										<td class="td1"><span class="style1">*</span>&nbsp;二级名称</td>
										<td class="td2">
											<input class="easyui-textbox" id="PM_fLevName2" required="required" name="fLevName2"  value="${bean.fLevName2 }" data-options="validType:'length[1,20]'" style="width: 150;height:25px" />
										</td>
										<td style="width: 50px">&nbsp;</td>
										<td class="td1"><span class="style1">*</span>&nbsp;二级代码</td>
										<td class="td2">
											<input class="easyui-textbox"   id="PM_fLevCode2" name="fLevCode2"  data-options="validType:'length[1,20]'" style="width: 150;height:25px" value="${bean.fLevCode2}"/>
										</td>
									</tr>	
									 <tr class="trbody">
									 	<%-- <td class="td1"><span class="style1">*</span>&nbsp;功能分类</td>
										<td class="td2">
											<input class="easyui-combobox" required="required" name="fFunctionClass" id="PM_fFunctionClass" value="${bean.fFunctionClass }" data-options="prompt:'--请选择--',valueField:'fecCode',
												textField:'fecName',
												method:'post',
												url:'${base}/ProMgrLevel/lookupsJsonFFunctionClass',
												onSelect:function(item){
													$('#PM_fFunctionClass').val(item.fecCode)
													$('#PM_fEcName').val(item.fecName)
												}
												" style="width: 150;height:25px" />
										</td> --%>
										<td class="td1">&nbsp;项目类型</td>
										<td class="td2">
											<input id="PM_fProType" name="fProType"  value="${bean.fProType }" data-options="url:'${base}/ProMgrLevel/lookupsJson?parentCode=PRO-CLASS&selected=${bean.fProType }',method:'POST',valueField:'code',textField:'text',editable:false" class="easyui-combobox" style="width: 150;height:25px" />
										</td>
									</tr> 
									<%-- <tr class="trbody">
										<td class="td1"><span class="style1">*</span>&nbsp;密级</td>
										<td class="td2">
											<input id="PM_fSecrecyGrade" name="fSecrecyGrade"  value="${bean.fSecrecyGrade }"  data-options="url:'${base}/ProMgrLevel/lookupsJson?parentCode=mj&selected=${bean.fSecrecyGrade }',method:'POST',valueField:'code',textField:'text',editable:false" class="easyui-combobox" style="width: 150;height:25px" />
										</td>
										<td style="width: 50px">&nbsp;</td>
									</tr> --%>
								</table>	
			...
			<div data-options="region:'south'" style="height: 51px;width: 100%; border: 0px;">
					<div style="text-align: center;">
						<c:if test="${openType=='add'||openType=='edit'}">
							<a href="javascript:void(0)" onclick="PM2AddEditForm();">
